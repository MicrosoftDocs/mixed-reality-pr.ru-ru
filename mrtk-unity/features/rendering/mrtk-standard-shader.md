...
стандартная система заливки мртк использует один гибкий шейдер, который может обеспечить визуальные элементы, аналогичные стандартному шейдеру Unity, реализовывать принципы [система Fluent Design](https://www.microsoft.com/design/fluent/) и продолжать работать на устройствах смешанной реальности.
...
| Особые настройки         | Позволяет управлять каждым аспектом режима рендеринга вручную. Только для расширенного использования.   |
...
чтобы выполнить обновление мртк, выберите пункт " **смешанная реальность набор средств-> служебные программы-> обновить мртк стандартный шейдер для упрощенного конвейера визуализации** ".
...
Также можно использовать инструмент для МРТК текстуры. чтобы открыть средство, выберите " **смешанная реальность набор средств-> служебные программы-> средство объединения текстур** , которое откроет следующее окно:
...
* [Светлое наведение](hover-light.md)
* [Неблизкое освещение](proximity-light.md)
* [Примитив обрезки](clipping-primitive.md)
