...
---
# <a name="eye-tracking-examples-overview"></a>Обзор примеров отслеживания взгляда
...
Imagine, что вы читаете какую-либо информацию на удаленном дисплее или на устройстве чтения и в конце отображаемого текста, текст автоматически прокручивается, чтобы показать больше содержимого.
...
обратите внимание, что для использования примеров отслеживания взгляда на устройстве требуется HoloLens 2 и пример пакета приложения, который создается с помощью функции "взгляд ввода" в AppXManifest пакета.

Чтобы использовать эти образцы отслеживания взгляда на устройстве, перед созданием приложения в Visual Studio необходимо выполнить следующие [действия](../input/eye-tracking/eye-tracking-basic-setup.md#testing-your-unity-app-on-a-hololens-2) .
...
чтобы загрузить аддитивные сцены во время выполнения, необходимо сначала добавить эти сцены в _сборку Параметры — > сцены в меню сборка_ .
...
после добавления сцен отслеживания взгляда в сборку Параметры и загрузки _эйетраккингдемо-00-рутсцене_ есть еще одна вещь, которую вы можете проверить: — это сценарий _онлоадстартсцене_ , прикрепленный к _микседреалитибасиксценесетуп_ GameObject. Это позволяет корневой сцене понять, какая демонстрационная сцена должна быть загружена в первую очередь.
...
однако это очень удобно для тестирования базовой функциональности перед итерацией проекта, развернув его на HoloLens 2 устройстве.
...
обратите внимание, что загрузка вспомогательной сцены, например, не является постоянной: это означает, что при развертывании приложения на HoloLens 2 устройстве будет загружена только корневая сцена (предполагая, что она отображается в верхней части Параметры сборки).
