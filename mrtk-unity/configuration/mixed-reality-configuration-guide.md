---
title: Руководство по настройке профиля MRTK
...
ms.locfileid: "115215004"
---
# <a name="mrtk-profile-configuration-guide"></a>Руководство по настройке профиля MRTK

смешанная реальность набор средств централизует как можно больше конфигурации, необходимой для управления набором средств (за исключением истинной среды выполнения).

Это простое пошаговое руководство для каждого экрана профиля конфигурации, доступного для набора средств.

## <a name="the-main-mixed-reality-toolkit-configuration-profile"></a>главный профиль конфигурации набор средств смешанной реальности

главный профиль конфигурации, присоединенный к *микседреалититулкит* GameObject в сцене, предоставляет главную точку входа для набор средств в проекте.

> [!NOTE]
> смешанная реальность набор средств "блокирует" экраны конфигурации по умолчанию, чтобы гарантировать, что у вас всегда есть общая начальная точка для проекта, и рекомендуется приступить к определению собственных параметров по мере развития проекта. Конфигурация МРТК не редактируется в режиме воспроизведения.
...
все профили "default" для набор средств смешанной реальности можно найти в проекте SDK в папке assets/мртк/SDK/profiles.
...
при открытии главного профиля конфигурации набор средств смешанной реальности в инспекторе отображается следующий экран:
...
- [рекомендации по настройке профиля набор средств смешанной реальности](#mrtk-profile-configuration-guide)
  - [главный профиль конфигурации набор средств смешанной реальности](#the-main-mixed-reality-toolkit-configuration-profile)
...
этот параметр, расположенный на главной странице настройки набор средств смешанной реальности, определяет операцию по умолчанию для [масштаба среды Mixed reality](/windows/mixed-reality/coordinate-systems-in-unity) для проекта.
...
Project смешанной реальности предоставляет надежную и хорошо обученную входную систему для маршрутизации всех событий ввода вокруг проекта, выбранного по умолчанию.
...
Project смешанной реальности предоставляет полнофункциональную систему телетранспорта для управления событиями в проекте, которая выбрана по умолчанию.
...
Project Mixed Reality предоставляет перестроенную систему пространственной информации для работы с системами пространственного сканирования в проекте, выбранном по умолчанию.

<img src="../features/images/mixed-reality-toolkit-configuration-profile-screens/MRTK_SpatialAwarenessSystemSelection.png" width="650px" alt="Spatial Awareness settings 1" style="display:block;">

конфигурация пространственной связи "смешанная реальность набор средств" позволяет настраивать способ запуска системы, независимо от того, запускается ли она автоматически при запуске приложения или более поздней версии, а также при установке экстентов для поля представления.
...
указатели можно также выработать в активной сцене с помощью одного из многих компонентов строк, предоставляемых набор средств смешанной реальности, или любого собственного элемента, если они реализуют интерфейс мртк имикседреалитипоинтер.
...
> [!NOTE]
> текущая реализация жестов предназначена только для HoloLens и будет расширена для других систем по мере их добавления в набор средств в будущем (даты пока не указаны).
...
1. общие Параметры — "поведение при запуске", настроенное на автоматический запуск или запуск вручную, определяет, следует ли инициализировать кэйвордрекогнизер при запуске системы ввода или позволить проекту определить, когда следует инициализировать кэйвордрекогнизер. "Уровень достоверности распознавания" используется для инициализации [API Кэйвордрекогнизер](https://docs.unity3d.com/ScriptReference/Windows.Speech.KeywordRecognizer-ctor.html) Unity.
...
> [!IMPORTANT]
> в настоящее время система поддерживает только распознавание речи при работе на Windows 10 платформах, например HoloLens и Windows 10 desktop и будет расширена для других систем, так как они будут добавлены в мртк в будущем (пока не даты).
...
одним из основных экранов настройки набор средств смешанной реальности является возможность настраивать и сопоставлять различные типы контроллеров, которые могут использоваться в проекте.
...
инспекторы служб можно включить, установив флажок *использовать инспекторы служб* в разделе *редактор Параметры* в профиле конфигурации.

### <a name="depth-buffer-renderer"></a>Модуль подготовки буфера глубины

Совместное использование буфера глубины с некоторыми платформами смешанной реальности может повысить [голограмму стабилизации](../performance/hologram-stabilization.md). например, Windows Mixed Realityная платформа может изменить отображаемую сцену на пиксель, чтобы учитывать незначительные движения головок в течение времени, когда оно заняло отрисовку кадра. Однако для этих методов требуются буферы глубины с точными данными, чтобы понять, где и насколько далеко от пользователя.

чтобы обеспечить визуализацию сцены всех необходимых данных в буфер глубины, разработчики могут переключать функцию *буфера глубины рендеринга* в *редакторе Параметры* в профиле конфигурации. Это займет текущий буфер глубины и выводит его в виде цвета в представление сцены, применяя к основной камере результат последующей обработки [`DepthBufferRenderer`](xref:Microsoft.MixedReality.Toolkit.Rendering.DepthBufferRenderer) .
