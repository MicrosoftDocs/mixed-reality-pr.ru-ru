---
title: Обновление с HoloToolkit
...
ms.locfileid: "115197414"
---
# <a name="upgrading-from-holotoolkit"></a>Обновление с HoloToolkit
...
| Установка                     | Поместите InputManager в сцену. | Включите входную систему в [профиле конфигурации](../configuration/mixed-reality-configuration-guide.md) и укажите конкретный тип входной системы. |
| Конфигурация             | Настроено в инспекторе для каждого отдельного скрипта в сцене. | Настраивается с помощью входного системного профиля и связанного с ним профиля, приведенного ниже. |
...
|        Методы                    | ХТК 2017 |  МРТК v2  |
|---------------------------|----------|-----------|
| Установка                     | удалите маинкамера, добавьте микседреалитикамерапарент/микседреалитикамера/хололенскамера prefab в сцену **или** используйте смешанную реальность набор средств > настроить > применить сцену смешанной реальности Параметры пункт меню. | маинкамера, родительская микседреалитиплайспаце с помощью смешанной реальности набор средств > добавить в сцену и настроить... |
| Конфигурация             | Настройка параметров камеры, выполненная в экземпляре prefab. | Параметры камеры, настроенные в [профиле камеры смешанной реальности](xref:Microsoft.MixedReality.Toolkit.MixedRealityCameraProfile). |
...
|         Методы                   | ХТК 2017 |  МРТК v2  |
|---------------------------|----------|-----------|
| Установка                     | Добавьте Спичинпутсаурце в сцену. | служба ключевых слов (например, Windows диспетчер речевого ввода) должна быть добавлена в поставщики данных входной системы. |
| Конфигурация             | Распознанные ключевые слова настраиваются в инспекторе Спичинпутсаурце. | Ключевые слова настраиваются в [профиле голосовых команд смешанной реальности](../features/input/speech.md). |
...
|         Методы                   | ХТК 2017 |  МРТК v2  |
|---------------------------|----------|-----------|
| Установка                     | Добавьте Диктатионинпутманажер в сцену. | для поддержки диктовки требуется служба (например, Windows "диспетчер ввода диктофона"), добавляемая в поставщики данных входной системы. |
...
|         Методы                   | ХТК 2017 |  МРТК v2  |
|---------------------------|----------|-----------|
| Установка                     | Добавьте prefab Спатиалмаппинг в сцену. | включите систему пространственной осведомленности в профиле конфигурации и добавьте пространственный наблюдатель (например, наблюдатель пространственной сетки Windows Mixed Reality) к поставщикам данных системы пространственной связи. |
| Конфигурация             | Настройте экземпляр сцены в инспекторе. | Настройте параметры для каждого профиля пространственного наблюдателя. |

### <a name="planes"></a>Плоскост

|         Методы                   | ХТК 2017 |  МРТК v2  |
|---------------------------|----------|-----------|
| Установка                     | Используйте `SurfaceMeshesToPlanes` скрипт. | Еще не реализовано. |

### <a name="spatial-understanding"></a>Пространственное понимание

|       Методы                      | ХТК 2017 |  МРТК v2  |
|---------------------------|----------|-----------|
| Установка                     | Добавьте prefab Спатиалундерстандинг в сцену. | Еще не реализовано. |
...
|         Методы                   | ХТК 2017 |  МРТК v2  |
|---------------------------|----------|-----------|
| Установка                     | Добавьте `BoundaryManager` скрипт в сцену. | Включите в профиле конфигурации систему границ. |
...
|             Методы               | ХТК 2017 |  МРТК v2  |
|---------------------------|----------|-----------|
| Установка                     | Общий доступ к службе: Добавьте в сцену общий доступ prefab. Унет: Используйте пример Шарингвисунет. | ведутся работы |
...
| Подсказка             | [Подсказка](https://github.com/Microsoft/MixedRealityToolkit-Unity/blob/htk_release/Assets/HoloToolkit-Examples/UX/Readme/README_TooltipExample.md) | [Подсказка](../features/ux-building-blocks/tooltip.md) |
