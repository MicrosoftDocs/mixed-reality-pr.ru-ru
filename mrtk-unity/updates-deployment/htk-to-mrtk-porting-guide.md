...
---
# <a name="porting-guide"></a>Руководство по портированию

Рекомендации по миграции из набора средств HoloLens (ХТК) в набор средств для смешанной реальности (МРТК).
...
|        Методы                    | ХТК 2017 |  МРТК v2  |
|---------------------------|----------|-----------|
| Настройка                     | Удалите Маинкамера, добавьте Микседреалитикамерапарент/Микседреалитикамера/Хололенскамера prefab в сцену **или** используйте набор средств для смешанной реальности > настроить > применить пункт меню "Параметры смешанной сцены". | Маинкамера, родительская под Микседреалитиплайспаце с помощью набора средств Mixed Reality > добавить в сцену и настроить... |
...
|         Методы                   | ХТК 2017 |  МРТК v2  |
|---------------------------|----------|-----------|
| Настройка                     | Добавьте Спичинпутсаурце в сцену. | Служба ключевых слов (например, диспетчер речевого ввода Windows) должна быть добавлена в поставщики данных входной системы. |
...
|         Методы                   | ХТК 2017 |  МРТК v2  |
|---------------------------|----------|-----------|
| Настройка                     | Добавьте Диктатионинпутманажер в сцену. | Для поддержки диктовки требуется служба (например, диспетчер ввода Windows диктофона), добавляемая в поставщики данных входной системы. |
...
|         Методы                   | ХТК 2017 |  МРТК v2  |
|---------------------------|----------|-----------|
| Настройка                     | Добавьте prefab Спатиалмаппинг в сцену. | Включите систему пространственной осведомленности в профиле конфигурации и добавьте пространственный наблюдатель (например, наблюдатель пространственной сетки Windows Mixed Reality) к поставщикам данных системы пространственной информации. |
...
| неарфаде | Встроенный [шейдер стандартного набора средств для смешанной реальности](../features/rendering/mrtk-standard-shader.md) |
