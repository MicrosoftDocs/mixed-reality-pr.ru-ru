...
description: Создайте настраиваемое приложение holographic для удаленного взаимодействия, чтобы отобразить содержимое, отображаемое на удаленном компьютере, в HoloLens 2.
...
ms.openlocfilehash: dde0ce3b824c80feda47c247cad4713717c8f9c0
ms.sourcegitcommit: bea83261bf9ce7a27a618e5bc54dc4d7711f5435
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 10/19/2021
ms.locfileid: "130158452"
---
...
с помощью удаленного плеера holographic приложение отображает holographic-содержимое, [отображаемое](../advanced-concepts/rendering-overview.md) на настольном компьютере или устройстве UWP, например Xbox One с доступом к дополнительным системным ресурсам. Приложение с удаленным проигрывателем holographic передает входные данные удаленному приложению holographic Remoting и получает иммерсивное представление в виде видео-и звукового потока. Подключение устанавливается с использованием стандартного Wi-Fi. чтобы создать приложение проигрывателя, используйте пакет NuGet, чтобы добавить holographic удаленное взаимодействие в приложение UWP. Затем напишите код, обрабатывающий соединение и отображающий иммерсивное представление. 
...
1. В обозреватель решений щелкните правой кнопкой мыши файл **Package. appxmanifest** и выберите команду **Открыть с помощью...**
...
Когда приложение игрока будет готово к отрисовке содержимого, можно установить подключение к удаленному приложению.
...
>[!IMPORTANT]
> Начиная с версии [2.1.0](holographic-remoting-version-history.md#v2.1.0) , в пользовательском проигрывателе можно использовать репроекцию с помощью удаленного взаимодействия.
...
## <a name="optional-over-rendering"></a>Необязательно: Over-Rendering

Holographic удаленное взаимодействие прогнозирует, где заголовком пользователя будет на момент отображения на экране отображаемых изображений. Однако этот прогноз является приближением. Таким образом, прогнозируемое окно просмотра в удаленном приложении и позднее фактическое окно просмотра в приложении проигрывателя может отличаться. Более надежные отклонения (например, из-за непредсказуемого движения) могут привести к появлению черных регионов на границах просмотра фрустум.
Начиная с версии [2.6.0](holographic-remoting-version-history.md#v2.6.0) можно использовать Over-Rendering, чтобы уменьшить черные регионы и улучшить качество визуального элемента, искусственно увеличивая окно просмотра за пределами фрустум просмотра.

Over-Rendering можно включить с помощью ```PlayerContext::ConfigureOverRendering``` .

Параметр ```OverRenderingConfig``` задает увеличение размера в долях до фактического окна просмотра, чтобы прогнозируемое окно просмотра стало более большим и менее вырезанием.
При увеличении размера окна просмотра уменьшается плотность пикселя, поэтому Оверрендерингконфиг позволяет увеличить разрешение.
Если увеличение окна просмотра равно увеличению разрешения, то плотность пикселя остается неизменной.
```OverRenderingConfig``` определяется следующим образом.
...
Начиная с версии [2.7.0](holographic-remoting-version-history.md#v2.7.0) . Синхронизация системы координат может использоваться для согласования пространственных данных между проигрывателем и удаленным приложением.
