...
[Удаленная отрисовка Azure (ARR)](/azure/remote-rendering/) — это служба, которая позволяет преобразовывать для просмотра трехмерные модели высокой сложности в реальном времени и выполнять их потоковую передачу на устройство. Сейчас ARR предоставляется в общедоступной предварительной версии. Ее также можно использовать в проектах Unity или с собственным кодом C++, предназначенных для HoloLens 2 или компьютеров с Windows.
...
![Виртуальное наложение привязок объектов с помощью Azure Object Anchors на примере открытого двигателя автомобиля](images/aoa-img-01.png)
