...
---
# <a name="updating-2d-uwp-apps-for-windows-mixed-reality"></a>Обновление двумерных приложений UWP для Windows Mixed Reality
...
Если Visual Studio не используется в среде разработки, можно открыть **AppXManifest.xml** в любом текстовом редакторе, чтобы убедиться, что вы используете **Windows. Universal** *TargetDeviceFamily*.
...
* Убедитесь, что у вас [установлен эмулятор HoloLens](../install-the-tools.md) .
...
Теперь, когда приложение UWP работает на впечатляющих гарнитурах и (или) HoloLens в виде двухмерной голограммы, далее мы будем уверены, что он выглядит прекраснее. Следует учесть ряд факторов:
