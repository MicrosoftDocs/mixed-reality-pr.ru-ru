---
title: 310. HoloLens (1-го поколения) и Azure — обнаружение объектов
...
ms.locfileid: "121905748"
---
# <a name="hololens-1st-gen-and-azure-310-object-detection"></a>HoloLens (1-й общий) и Azure 310: обнаружение объектов
...
- ПК для разработки
- [Windows 10 Fall Creators Update (или более поздней версии) с включенным режимом разработчика](../../install-the-tools.md#installation-checklist-for-hololens)
- [последний пакет SDK для Windows 10](../../install-the-tools.md#installation-checklist-for-hololens)
...
2.  Настройте и протестируйте HoloLens. если вам нужна поддержка, [обратитесь к статье о настройке HoloLens](/hololens/hololens-setup).
...
6.  Принимаю условия, вы теперь в разделе *Мои проекты* . Щелкните **New Project (создать**).
...
7.  На правой стороне появится вкладка, в которой будет предложено указать некоторые поля для проекта.
    ...
    4.  задайте **типы Project** как **обнаружение объектов (предварительная версия)**.
...
6.  после выбора всех изображений, с которыми вы хотите обучить проект, нажмите **Upload файлы** . Файлы начнут отправляться. Получив подтверждение отправки, нажмите кнопку **Готово**.
...
14. когда вы щелкнули **url-адрес прогноза**, откройте *Блокнот*, скопируйте и вставьте **URL-адрес** (также называемый **прогнозной конечной точкой**) и **ключ прогноза службы**, чтобы его можно было извлечь, когда он понадобится позже в коде.
...
3.  При открытом Unity стоит проверить, что **Редактор скриптов** по умолчанию имеет значение **Visual Studio**. Перейдите к разделу **изменение*  >  *настроек** , а затем в новом окне перейдите к разделу **Внешние инструменты**. Измените **Редактор внешних скриптов** на **Visual Studio**. Закройте окно **настройки** .

    ![](images/AzureLabs-Lab310-23.png)

4.  затем перейдите в раздел **файл > сборка Параметры** и переключите **платформу** на *универсальная платформа Windows*, а затем нажмите кнопку **коммутатора платформы** .

    ![](images/AzureLabs-Lab310-24.png)

5.  в том же окне **Параметры сборки** убедитесь, что установлены следующие значения:
...
6.  в том же окне **Параметры сборки** нажмите кнопку **Параметры проигрывателя** , чтобы открыть связанную панель в пространстве, где расположен **инспектор** .
...
4.  откройте папку " **сцены** " на **панели Project** и дважды щелкните **обждетектионсцене**, чтобы загрузить сцену, которая будет использоваться для этого курса.
...
1.  щелкните правой кнопкой мыши **папку Asset**, расположенную на **панели Project**, и выберите команду **создать**  >  **папку**. Вызовите **скрипты** папки.
...
4.  Дважды щелкните новый скрипт **кустомвисионаналисер** , чтобы открыть его с помощью **Visual Studio.**
...
6.  В классе **кустомвисионаналисер** добавьте следующие переменные:
    ...
    > [!NOTE]
    > Убедитесь, что вы вставили **ключ прогноза службы** в переменную **Предиктионкэй** , а **Прогноз-конечную точку —** в переменную **предиктионендпоинт** . вы скопировали их в [Блокнот ранее, в главе 2, шаге 14](#chapter-2---training-your-custom-vision-project).
...
2.  Дважды щелкните новый скрипт **кустомвисионобжектс** , чтобы открыть его с помощью **Visual Studio.**
...
2.  Дважды щелкните новый скрипт **спатиалмаппинг** , чтобы открыть его с помощью **Visual Studio.**
...
2.  Дважды щелкните новый скрипт **газекурсор** , чтобы открыть его с помощью **Visual Studio.**
...
2.  Дважды щелкните новый скрипт **сценеорганисер** , чтобы открыть его с помощью **Visual Studio.**
...
*   запись изображения с помощью HoloLens камеры и сохранение его в папке *приложения* .
...
3.  Дважды щелкните новый скрипт **имажекаптуре** , чтобы открыть его с помощью **Visual Studio.**
...
3.  на **панели Project** откройте **папку Prefabs**, перетащите **метку** prefab в область входные данные целевого объекта *метки* пусто, в скрипте **сценеорганисер** , который вы только что добавили к *основной камере*, как показано на рисунке ниже:
...
Теперь вы готовы к созданию приложения в качестве решения UWP, которое вы сможете развернуть на Microsoft HoloLens. Чтобы начать процесс сборки:

1.  последовательно выберите **файл > сборка Параметры**.
...
7.  чтобы выполнить развертывание на Microsoft HoloLens, вам потребуется IP-адрес этого устройства (для удаленного развертывания) и убедиться, что в нем также установлен **режим разработчика** . Для этого выполните следующие действия.
    ...
    4.  затем вернитесь к **Параметры**, а затем **обновите & безопасность**  >  **для разработчиков** .

    5.  Задайте **режим разработчика** *на*.

8.  Перейдите к новой сборке Unity (папка **приложения** ) и откройте файл решения с помощью **Visual Studio**.
...
11. Перейдите в меню " **Сборка** " и щелкните " **Развернуть решение** ", чтобы загружать неопубликованные приложение на HoloLens.

12. теперь приложение должно появиться в списке установленных приложений на Microsoft HoloLens, готовых к запуску.
...
> [!NOTE]
> помещенный текст метки может не отображаться рядом с объектом, когда Microsoft HoloLens датчики и (или) спатиалтраккингкомпонент в Unity не могут поместить соответствующие конфликты в зависимости от реальных объектов. Если это так, попробуйте использовать приложение на другой поверхности.
