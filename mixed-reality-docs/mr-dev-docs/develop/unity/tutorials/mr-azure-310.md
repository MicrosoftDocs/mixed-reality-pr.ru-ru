...
<span data-ttu-id="3c37d-127">Курс также охватывает ручную загрузку изображений, Создание тегов и обучение службы, чтобы распознать различные объекты (в предоставленном примере — чашку), установив *границу* в рамках отправляемого изображения.</span><span class="sxs-lookup"><span data-stu-id="3c37d-127">The course will also cover manually uploading images, creating tags, and training the Service, to recognize different objects (in the provided example, a cup), by setting the *Boundary Box* within the image you submit.</span></span> 
...
## <a name="before-you-start"></a><span data-ttu-id="3c37d-152">Перед началом работы</span><span class="sxs-lookup"><span data-stu-id="3c37d-152">Before you start</span></span>
...
7. <span data-ttu-id="3c37d-235">На этой панели необходимо проверить несколько параметров:</span><span class="sxs-lookup"><span data-stu-id="3c37d-235">In this panel, a few settings need to be verified:</span></span>
    ...
    1.  <span data-ttu-id="3c37d-236">На вкладке **другие параметры** выполните следующие действия.</span><span class="sxs-lookup"><span data-stu-id="3c37d-236">In the **Other Settings** tab:</span></span>
        ...
        3. <span data-ttu-id="3c37d-239">**Уровень совместимости API** должен быть **.NET 4,6**.</span><span class="sxs-lookup"><span data-stu-id="3c37d-239">**API Compatibility Level** should be **.NET 4.6**.</span></span>
...
<span data-ttu-id="3c37d-290">Чтобы создать этот класс, сделайте следующее:</span><span class="sxs-lookup"><span data-stu-id="3c37d-290">To create this class:</span></span>
...
## <a name="chapter-6---create-the-customvisionobjects-class"></a><span data-ttu-id="3c37d-308">Глава 6. Создание класса Кустомвисионобжектс</span><span class="sxs-lookup"><span data-stu-id="3c37d-308">Chapter 6 - Create the CustomVisionObjects class</span></span>
...
4.  <span data-ttu-id="3c37d-329">Затем добавьте следующие переменные в класс **спатиалмаппинг** над методом **Start ()** :</span><span class="sxs-lookup"><span data-stu-id="3c37d-329">Then, add the following variables inside the **SpatialMapping** class, above the **Start()** method:</span></span>
...
6.  <span data-ttu-id="3c37d-331">Удалите метод **Update ()** .</span><span class="sxs-lookup"><span data-stu-id="3c37d-331">Delete the **Update()** method.</span></span>
...
<span data-ttu-id="3c37d-346">Этот класс будет:</span><span class="sxs-lookup"><span data-stu-id="3c37d-346">This class will:</span></span>
...
8.  <span data-ttu-id="3c37d-360">Добавьте метод **финалиселабел ()** .</span><span class="sxs-lookup"><span data-stu-id="3c37d-360">Add the **FinaliseLabel()** method.</span></span> <span data-ttu-id="3c37d-361">Он отвечает за следующее:</span><span class="sxs-lookup"><span data-stu-id="3c37d-361">It is responsible for:</span></span>
    ...
    *   <span data-ttu-id="3c37d-364">Настройка глубины метки с помощью Райкаст к *ограничивающему прямоугольнику*, который должен конфликтовать с объектом в реальном мире.</span><span class="sxs-lookup"><span data-stu-id="3c37d-364">Adjusting the label depth by using a Raycast towards the *Bounding Box*, which should collide against the object in the real world.</span></span>
    * <span data-ttu-id="3c37d-365">Сброс процесса отслеживания, позволяющий пользователю записать другой образ.</span><span class="sxs-lookup"><span data-stu-id="3c37d-365">Resetting the capture process to allow the user to capture another image.</span></span>
...
9.  <span data-ttu-id="3c37d-366">Добавьте метод **калкулатебаундингбокспоситион ()** , который содержит ряд вычислений, необходимых для преобразования координат *ограничивающего прямоугольника* , полученных из службы, и повторного создания их пропорционально четырем.</span><span class="sxs-lookup"><span data-stu-id="3c37d-366">Add the **CalculateBoundingBoxPosition()** method, which hosts a number of calculations necessary to translate the *Bounding Box* coordinates retrieved from the Service and recreate them proportionally on the quad.</span></span>
...
10. <span data-ttu-id="3c37d-367">Не забудьте сохранить изменения в **Visual Studio** перед возвратом в **Unity**.</span><span class="sxs-lookup"><span data-stu-id="3c37d-367">Be sure to save your changes in **Visual Studio**, before returning to **Unity**.</span></span>

    > [!IMPORTANT]
    > <span data-ttu-id="3c37d-368">Прежде чем продолжить, откройте класс **кустомвисионаналисер** и в методе **аналиселастимажекаптуред ()** *раскомментируйте* следующие строки:</span><span class="sxs-lookup"><span data-stu-id="3c37d-368">Before you continue, open the **CustomVisionAnalyser** class, and within the **AnalyseLastImageCaptured()** method, *uncomment* the following lines:</span></span>
    >
    >   ```csharp
    >   // Create a texture. Texture size does not matter, since 
    >   // LoadImage will replace with the incoming image size.
...
> [!NOTE]
> <span data-ttu-id="3c37d-369">Не беспокойтесь о сообщении о том, что класс **имажекаптуре** не найден, вы создадите его в следующей главе.</span><span class="sxs-lookup"><span data-stu-id="3c37d-369">Do not worry about the **ImageCapture** class 'could not be found' message, you will create it in the next chapter.</span></span>


## <a name="chapter-10---create-the-imagecapture-class"></a><span data-ttu-id="3c37d-370">Глава 10. Создание класса Имажекаптуре</span><span class="sxs-lookup"><span data-stu-id="3c37d-370">Chapter 10 - Create the ImageCapture class</span></span>

<span data-ttu-id="3c37d-371">Следующий класс, который предстоит создать, — это класс **имажекаптуре** .</span><span class="sxs-lookup"><span data-stu-id="3c37d-371">The next class you are going to create is the **ImageCapture** class.</span></span>

<span data-ttu-id="3c37d-372">Этот класс отвечает за:</span><span class="sxs-lookup"><span data-stu-id="3c37d-372">This class is responsible for:</span></span>

*   <span data-ttu-id="3c37d-373">Запись изображения с помощью камеры HoloLens и сохранение его в папке *приложения* .</span><span class="sxs-lookup"><span data-stu-id="3c37d-373">Capturing an image using the HoloLens camera and storing it in the *App* folder.</span></span>
*   <span data-ttu-id="3c37d-374">Обработка жестов *касания* от пользователя.</span><span class="sxs-lookup"><span data-stu-id="3c37d-374">Handling *Tap* gestures from the user.</span></span>

<span data-ttu-id="3c37d-375">Чтобы создать этот класс, сделайте следующее:</span><span class="sxs-lookup"><span data-stu-id="3c37d-375">To create this class:</span></span>

1.  <span data-ttu-id="3c37d-376">Перейдите к созданной ранее папке **Scripts** .</span><span class="sxs-lookup"><span data-stu-id="3c37d-376">Go to the **Scripts** folder you created previously.</span></span>

2.  <span data-ttu-id="3c37d-377">Щелкните правой кнопкой мыши внутри папки и выберите команду **создать**  >  **\# Сценарий C**.</span><span class="sxs-lookup"><span data-stu-id="3c37d-377">Right-click inside the folder, then click **Create** > **C\# Script**.</span></span> <span data-ttu-id="3c37d-378">Назовите сценарий **имажекаптуре**.</span><span class="sxs-lookup"><span data-stu-id="3c37d-378">Name the script **ImageCapture**.</span></span>

3.  <span data-ttu-id="3c37d-379">Дважды щелкните новый скрипт **имажекаптуре** , чтобы открыть его в **Visual Studio.**</span><span class="sxs-lookup"><span data-stu-id="3c37d-379">Double-click on the new **ImageCapture** script to open it with **Visual Studio.**</span></span>

4.  <span data-ttu-id="3c37d-380">Замените пространства имен в верхней части файла следующим:</span><span class="sxs-lookup"><span data-stu-id="3c37d-380">Replace the namespaces at the top of the file with the following:</span></span>
...
5.  <span data-ttu-id="3c37d-381">Затем добавьте следующие переменные в класс **имажекаптуре** над методом **Start ()** :</span><span class="sxs-lookup"><span data-stu-id="3c37d-381">Then add the following variables inside the **ImageCapture** class, above the **Start()** method:</span></span>
...
6.  <span data-ttu-id="3c37d-382">Теперь необходимо добавить код для методов **"спящий" ()** и **"начало" ()** :</span><span class="sxs-lookup"><span data-stu-id="3c37d-382">Code for **Awake()** and **Start()** methods now needs to be added:</span></span>
...
7.  <span data-ttu-id="3c37d-383">Реализуйте обработчик, который будет вызываться при возникновении жеста касания:</span><span class="sxs-lookup"><span data-stu-id="3c37d-383">Implement a handler that will be called when a Tap gesture occurs:</span></span>
    ...
    > [!IMPORTANT]
    > <span data-ttu-id="3c37d-384">Когда курсор **горит зеленым**, это означает, что Камера доступна для получения образа.</span><span class="sxs-lookup"><span data-stu-id="3c37d-384">When the cursor is **green**, it means the camera is available to take the image.</span></span> <span data-ttu-id="3c37d-385">**Красный цвет** курсора означает, что камера занята.</span><span class="sxs-lookup"><span data-stu-id="3c37d-385">When the cursor is **red**, it means the camera is busy.</span></span>

8.  <span data-ttu-id="3c37d-386">Добавьте метод, используемый приложением для запуска процесса записи образа, и сохраните образ:</span><span class="sxs-lookup"><span data-stu-id="3c37d-386">Add the method that the application uses to start the image capture process and store the image:</span></span>
...
9.  <span data-ttu-id="3c37d-387">Добавьте обработчики, которые будут вызываться при записи фотографии и при ее готовности к анализу.</span><span class="sxs-lookup"><span data-stu-id="3c37d-387">Add the handlers that will be called when the photo has been captured and for when it is ready to be analyzed.</span></span> <span data-ttu-id="3c37d-388">Затем результат передается в **кустомвисионаналисер** для анализа.</span><span class="sxs-lookup"><span data-stu-id="3c37d-388">The result is then passed to the **CustomVisionAnalyser** for analysis.</span></span>
...
10. <span data-ttu-id="3c37d-389">Не забудьте сохранить изменения в **Visual Studio** перед возвратом в **Unity**.</span><span class="sxs-lookup"><span data-stu-id="3c37d-389">Be sure to save your changes in **Visual Studio**, before returning to **Unity**.</span></span>

## <a name="chapter-11---setting-up-the-scripts-in-the-scene"></a><span data-ttu-id="3c37d-390">Глава 11. Настройка сценариев в сцене</span><span class="sxs-lookup"><span data-stu-id="3c37d-390">Chapter 11 - Setting up the scripts in the scene</span></span>

<span data-ttu-id="3c37d-391">Теперь, когда вы написали весь код, необходимый для этого проекта, пора настроить скрипты в сцене и на Prefabs, чтобы они правильно ведут себя.</span><span class="sxs-lookup"><span data-stu-id="3c37d-391">Now that you have written all of the code necessary for this project, is time to set up the scripts in the scene, and on the prefabs, for them to behave correctly.</span></span>

1.  <span data-ttu-id="3c37d-392">В **редакторе Unity** на **панели Иерархия** выберите **основную камеру**.</span><span class="sxs-lookup"><span data-stu-id="3c37d-392">Within the **Unity Editor**, in the **Hierarchy Panel**, select the **Main Camera**.</span></span>
2.  <span data-ttu-id="3c37d-393">На **панели инспектора** с выбранной **основной камерой** щелкните **Добавить компонент**, найдите скрипт **сценеорганисер** и дважды щелкните, чтобы добавить его.</span><span class="sxs-lookup"><span data-stu-id="3c37d-393">In the **Inspector Panel**, with the **Main Camera** selected, click on **Add Component**, then search for **SceneOrganiser** script and double-click, to add it.</span></span>

    ![](images/AzureLabs-Lab310-38.png)

3.  <span data-ttu-id="3c37d-394">На **панели проект** откройте **папку Prefabs**, перетащите **метку** prefab в область входные данные целевой объект *метки* пусто, в скрипте **Сценеорганисер** , который вы только что добавили к *основной камере*, как показано на рисунке ниже:</span><span class="sxs-lookup"><span data-stu-id="3c37d-394">In the **Project Panel**, open the **Prefabs folder**, drag the **Label** prefab into the *Label* empty reference target input area, in the **SceneOrganiser** script that you have just added to the *Main Camera*, as shown in the image below:</span></span>

    ![](images/AzureLabs-Lab310-39.png)

4.  <span data-ttu-id="3c37d-395">На **панели Иерархия** выберите дочерний элемент **газекурсор** **основной камеры**.</span><span class="sxs-lookup"><span data-stu-id="3c37d-395">In the **Hierarchy Panel**, select the **GazeCursor** child of the **Main Camera**.</span></span>
5.  <span data-ttu-id="3c37d-396">На **панели инспектора** с выбранным **Газекурсор** щелкните **Добавить компонент**, затем найдите скрипт **газекурсор** и дважды щелкните, чтобы добавить его.</span><span class="sxs-lookup"><span data-stu-id="3c37d-396">In the **Inspector Panel**, with the **GazeCursor** selected, click on **Add Component**, then search for **GazeCursor** script and double-click, to add it.</span></span>

    ![](images/AzureLabs-Lab310-40.png)

6.  <span data-ttu-id="3c37d-397">Опять же, на **панели Иерархия** выберите дочерний элемент **спатиалмаппинг** **основной камеры**.</span><span class="sxs-lookup"><span data-stu-id="3c37d-397">Again, in the **Hierarchy Panel**, select the **SpatialMapping** child of the **Main Camera**.</span></span>
7.  <span data-ttu-id="3c37d-398">На **панели инспектора** с выбранным **Спатиалмаппинг** щелкните **Добавить компонент**, затем найдите скрипт **спатиалмаппинг** и дважды щелкните, чтобы добавить его.</span><span class="sxs-lookup"><span data-stu-id="3c37d-398">In the **Inspector Panel**, with the **SpatialMapping** selected, click on **Add Component**, then search for **SpatialMapping** script and double-click, to add it.</span></span>

    ![](images/AzureLabs-Lab310-41.png)

<span data-ttu-id="3c37d-399">Остальные скрипты, которые не были заданы, будут добавляться кодом в скрипте **сценеорганисер** во время выполнения.</span><span class="sxs-lookup"><span data-stu-id="3c37d-399">The remaining scripts thats you have not set will be added by the code in the **SceneOrganiser** script, during runtime.</span></span>

## <a name="chapter-12---before-building"></a><span data-ttu-id="3c37d-400">Глава 12-перед сборкой</span><span class="sxs-lookup"><span data-stu-id="3c37d-400">Chapter 12 - Before building</span></span>

<span data-ttu-id="3c37d-401">Чтобы выполнить тщательный тест приложения, необходимо загружать неопубликованные его на Microsoft HoloLens.</span><span class="sxs-lookup"><span data-stu-id="3c37d-401">To perform a thorough test of your application you will need to sideload it onto your Microsoft HoloLens.</span></span>

<span data-ttu-id="3c37d-402">Перед этим убедитесь в том, что:</span><span class="sxs-lookup"><span data-stu-id="3c37d-402">Before you do, ensure that:</span></span>

-  <span data-ttu-id="3c37d-403">Все параметры, упомянутые в [главе 3](#chapter-3---set-up-the-unity-project) , заданы правильно.</span><span class="sxs-lookup"><span data-stu-id="3c37d-403">All the settings mentioned in the [Chapter 3](#chapter-3---set-up-the-unity-project) are set correctly.</span></span>
- <span data-ttu-id="3c37d-404">**Сценеорганисер** скрипта прикреплен к **главному объекту Camera** .</span><span class="sxs-lookup"><span data-stu-id="3c37d-404">The script **SceneOrganiser** is attached to the **Main Camera** object.</span></span>
- <span data-ttu-id="3c37d-405">**Газекурсор** скрипта присоединен к объекту **газекурсор** .</span><span class="sxs-lookup"><span data-stu-id="3c37d-405">The script **GazeCursor** is attached to the **GazeCursor** object.</span></span>
- <span data-ttu-id="3c37d-406">**Спатиалмаппинг** скрипта присоединен к объекту **спатиалмаппинг** .</span><span class="sxs-lookup"><span data-stu-id="3c37d-406">The script **SpatialMapping** is attached to the **SpatialMapping** object.</span></span>
- <span data-ttu-id="3c37d-407">В [главе 5](#chapter-5---create-the-customvisionanalyser-class)шаг 6.</span><span class="sxs-lookup"><span data-stu-id="3c37d-407">In [Chapter 5](#chapter-5---create-the-customvisionanalyser-class), Step 6:</span></span>

    - <span data-ttu-id="3c37d-408">Убедитесь, что вы вставили **ключ прогноза службы** в переменную **предиктионкэй** .</span><span class="sxs-lookup"><span data-stu-id="3c37d-408">Make sure you insert your **Service Prediction Key** into the **predictionKey** variable.</span></span>
    - <span data-ttu-id="3c37d-409">Вы вставили **конечную точку прогнозирования** в класс **предиктионендпоинт** .</span><span class="sxs-lookup"><span data-stu-id="3c37d-409">You have inserted your **Prediction Endpoint** into the **predictionEndpoint** class.</span></span>

## <a name="chapter-13---build-the-uwp-solution-and-sideload-your-application"></a><span data-ttu-id="3c37d-410">Глава 13. Создание решения UWP и загружать неопубликованные приложения</span><span class="sxs-lookup"><span data-stu-id="3c37d-410">Chapter 13 - Build the UWP solution and sideload your application</span></span>

<span data-ttu-id="3c37d-411">Теперь вы готовы к созданию приложения в качестве решения UWP, которое можно будет развернуть на Microsoft HoloLens.</span><span class="sxs-lookup"><span data-stu-id="3c37d-411">You are now ready to build you application as a UWP Solution that you will be able to deploy on to the Microsoft HoloLens.</span></span> <span data-ttu-id="3c37d-412">Чтобы начать процесс сборки:</span><span class="sxs-lookup"><span data-stu-id="3c37d-412">To begin the build process:</span></span>

1.  <span data-ttu-id="3c37d-413">Выберите **файл > параметры сборки**.</span><span class="sxs-lookup"><span data-stu-id="3c37d-413">Go to **File > Build Settings**.</span></span>

2.  <span data-ttu-id="3c37d-414">**\# Проекты тактов Unity C**.</span><span class="sxs-lookup"><span data-stu-id="3c37d-414">Tick **Unity C\# Projects**.</span></span>

3.  <span data-ttu-id="3c37d-415">Щелкните **Добавить открытые сцены**.</span><span class="sxs-lookup"><span data-stu-id="3c37d-415">Click on **Add Open Scenes**.</span></span> <span data-ttu-id="3c37d-416">При этом в сборку будет добавлена открытая сцена.</span><span class="sxs-lookup"><span data-stu-id="3c37d-416">This will add the currently open scene to the build.</span></span>

    ![](images/AzureLabs-Lab310-42.png)

4.  <span data-ttu-id="3c37d-417">Щелкните **Построить**.</span><span class="sxs-lookup"><span data-stu-id="3c37d-417">Click **Build**.</span></span> <span data-ttu-id="3c37d-418">Unity запустит окно *проводника* , в котором необходимо создать, а затем выбрать папку для сборки приложения.</span><span class="sxs-lookup"><span data-stu-id="3c37d-418">Unity will launch a *File Explorer* window, where you need to create and then select a folder to build the app into.</span></span> <span data-ttu-id="3c37d-419">Создайте эту папку сейчас и назовите ее **app** Name.</span><span class="sxs-lookup"><span data-stu-id="3c37d-419">Create that folder now, and name it **App**.</span></span> <span data-ttu-id="3c37d-420">Затем выберите папку **приложения** и щелкните **выбрать папку**.</span><span class="sxs-lookup"><span data-stu-id="3c37d-420">Then with the **App** folder selected, click **Select Folder**.</span></span>

5.  <span data-ttu-id="3c37d-421">Unity начнет сборку проекта в папку **приложения** .</span><span class="sxs-lookup"><span data-stu-id="3c37d-421">Unity will begin building your project to the **App** folder.</span></span>

6.  <span data-ttu-id="3c37d-422">После того как Unity завершит сборку (может занять некоторое время), он откроет окно **проводника** в расположении сборки (проверьте панель задач, так как она может не всегда отображаться над окнами, но будет уведомлять о добавлении нового окна).</span><span class="sxs-lookup"><span data-stu-id="3c37d-422">Once Unity has finished building (it might take some time), it will open a **File Explorer** window at the location of your build (check your task bar, as it may not always appear above your windows, but will notify you of the addition of a new window).</span></span>

7.  <span data-ttu-id="3c37d-423">Чтобы выполнить развертывание в Microsoft HoloLens, вам потребуется IP-адрес этого устройства (для удаленного развертывания) и убедиться, что в нем также установлен **режим разработчика** .</span><span class="sxs-lookup"><span data-stu-id="3c37d-423">To deploy on to Microsoft HoloLens, you will need the IP Address of that device (for Remote Deploy), and to ensure that it also has **Developer Mode** set.</span></span> <span data-ttu-id="3c37d-424">Выполните указанные ниже действия.</span><span class="sxs-lookup"><span data-stu-id="3c37d-424">To do this:</span></span>

    1.  <span data-ttu-id="3c37d-425">Людьми HoloLens, откройте **Параметры**.</span><span class="sxs-lookup"><span data-stu-id="3c37d-425">Whilst wearing your HoloLens, open the **Settings**.</span></span>

    2.  <span data-ttu-id="3c37d-426">Выберите **Сетевые &**  >  **Wi-Fi**  >  **Дополнительные параметры** сети Интернет Wi-Fi</span><span class="sxs-lookup"><span data-stu-id="3c37d-426">Go to **Network & Internet** > **Wi-Fi** > **Advanced Options**</span></span>

    3.  <span data-ttu-id="3c37d-427">Запишите **IPv4** -адрес.</span><span class="sxs-lookup"><span data-stu-id="3c37d-427">Note the **IPv4** address.</span></span>

    4.  <span data-ttu-id="3c37d-428">Затем вернитесь к **параметрам** и **Обновите & безопасности**  >  **для разработчиков** .</span><span class="sxs-lookup"><span data-stu-id="3c37d-428">Next, navigate back to **Settings**, and then to **Update & Security** > **For Developers**</span></span>

    5.  <span data-ttu-id="3c37d-429">Задайте **режим разработчика** *на*.</span><span class="sxs-lookup"><span data-stu-id="3c37d-429">Set **Developer Mode** *On*.</span></span>

8.  <span data-ttu-id="3c37d-430">Перейдите к новой сборке Unity (папка **приложения** ) и откройте файл решения в **Visual Studio**.</span><span class="sxs-lookup"><span data-stu-id="3c37d-430">Navigate to your new Unity build (the **App** folder) and open the solution file with **Visual Studio**.</span></span>

9.  <span data-ttu-id="3c37d-431">В конфигурации решения выберите **Отладка**.</span><span class="sxs-lookup"><span data-stu-id="3c37d-431">In the Solution Configuration select **Debug**.</span></span>

10. <span data-ttu-id="3c37d-432">На платформе решения выберите **x86, удаленный компьютер**.</span><span class="sxs-lookup"><span data-stu-id="3c37d-432">In the Solution Platform, select **x86, Remote Machine**.</span></span> <span data-ttu-id="3c37d-433">Вам будет предложено вставить **IP-адрес** удаленного устройства (в данном случае — Microsoft HoloLens, который вы заметили).</span><span class="sxs-lookup"><span data-stu-id="3c37d-433">You will be prompted to insert the **IP address** of a remote device (the Microsoft HoloLens, in this case, which you noted).</span></span>

    ![](images/AzureLabs-Lab310-43.png)

11. <span data-ttu-id="3c37d-434">Перейдите в меню " **Сборка** " и щелкните " **Развернуть решение** ", чтобы загружать неопубликованные приложение в HoloLens.</span><span class="sxs-lookup"><span data-stu-id="3c37d-434">Go to the **Build** menu and click on **Deploy Solution** to sideload the application to your HoloLens.</span></span>

12. <span data-ttu-id="3c37d-435">Теперь приложение должно появиться в списке установленных приложений в Microsoft HoloLens, готовом к запуску.</span><span class="sxs-lookup"><span data-stu-id="3c37d-435">Your app should now appear in the list of installed apps on your Microsoft HoloLens, ready to be launched!</span></span>

### <a name="to-use-the-application"></a><span data-ttu-id="3c37d-436">Чтобы использовать приложение, выполните следующие действия.</span><span class="sxs-lookup"><span data-stu-id="3c37d-436">To use the application:</span></span>

* <span data-ttu-id="3c37d-437">Взгляните на объект, который вы обучили **Пользовательская служба визуального распознавания Azure, обнаружение объектов** и используйте **жест касания**.</span><span class="sxs-lookup"><span data-stu-id="3c37d-437">Look at an object, which you have trained with your **Azure Custom Vision Service, Object Detection**, and use the **Tap gesture**.</span></span>
* <span data-ttu-id="3c37d-438">Если объект успешно обнаружен, *текст метки* в мировом пространстве будет отображаться с именем тега.</span><span class="sxs-lookup"><span data-stu-id="3c37d-438">If the object is successfully detected, a world-space *Label Text* will appear with the tag name.</span></span>

> [!IMPORTANT]
> <span data-ttu-id="3c37d-439">Каждый раз, когда вы записываете фотографию и отправляете ее в службу, вы можете вернуться на страницу службы и переучить службу с новыми записанными образами.</span><span class="sxs-lookup"><span data-stu-id="3c37d-439">Every time you capture a photo and send it to the Service, you can go back to the Service page and retrain the Service with the newly captured images.</span></span> <span data-ttu-id="3c37d-440">В начале вам, возможно, придется исправить *ограничивающие рамки* , чтобы они были более точными и переучить службу.</span><span class="sxs-lookup"><span data-stu-id="3c37d-440">At the beginning, you will probably also have to correct the *Bounding Boxes* to be more accurate and retrain the Service.</span></span>

> [!NOTE]
> <span data-ttu-id="3c37d-441">Размещенный текст метки может не располагаться рядом с объектом, когда датчики Microsoft HoloLens и (или) Спатиалтраккингкомпонент в Unity не могут поместить соответствующие конфликты, связанные с реальными объектами мира.</span><span class="sxs-lookup"><span data-stu-id="3c37d-441">The Label Text placed might not appear near the object when the Microsoft HoloLens sensors and/or the SpatialTrackingComponent in Unity fails to place the appropriate colliders, relative to the real world objects.</span></span> <span data-ttu-id="3c37d-442">Если это так, попробуйте использовать приложение на другой поверхности.</span><span class="sxs-lookup"><span data-stu-id="3c37d-442">Try to use the application on a different surface if that is the case.</span></span>

## <a name="your-custom-vision-object-detection-application"></a><span data-ttu-id="3c37d-443">Пользовательское визуальное распознавание, приложение обнаружения объектов</span><span class="sxs-lookup"><span data-stu-id="3c37d-443">Your Custom Vision, Object Detection application</span></span>

<span data-ttu-id="3c37d-444">Поздравляем! вы создали приложение смешанной реальности, которое использует Пользовательское визуальное распознавание Azure, API обнаружения объектов, который может распознать объект из изображения, а затем выдать приблизительное место для этого объекта в трехмерном пространстве.</span><span class="sxs-lookup"><span data-stu-id="3c37d-444">Congratulations, you built a mixed reality app that leverages the Azure Custom Vision, Object Detection API, which can recognize an object from an image, and then provide an approximate position for that object in 3D space.</span></span>

![](images/AzureLabs-Lab310-00.png)

## <a name="bonus-exercises"></a><span data-ttu-id="3c37d-445">Дополнительные упражнения</span><span class="sxs-lookup"><span data-stu-id="3c37d-445">Bonus exercises</span></span>

### <a name="exercise-1"></a><span data-ttu-id="3c37d-446">Упражнение 1</span><span class="sxs-lookup"><span data-stu-id="3c37d-446">Exercise 1</span></span>

<span data-ttu-id="3c37d-447">При добавлении к текстовой метке используйте полупрозрачный куб, чтобы обернуть реальный объект в трехмерный *ограничивающий прямоугольник*.</span><span class="sxs-lookup"><span data-stu-id="3c37d-447">Adding to the Text Label, use a semi-transparent cube to wrap the real object in a 3D *Bounding Box*.</span></span>

### <a name="exercise-2"></a><span data-ttu-id="3c37d-448">Упражнение 2</span><span class="sxs-lookup"><span data-stu-id="3c37d-448">Exercise 2</span></span>

<span data-ttu-id="3c37d-449">Обучить Пользовательская служба визуального распознавания для распознавания большего числа объектов.</span><span class="sxs-lookup"><span data-stu-id="3c37d-449">Train your Custom Vision Service to recognize more objects.</span></span>

### <a name="exercise-3"></a><span data-ttu-id="3c37d-450">Упражнение 3</span><span class="sxs-lookup"><span data-stu-id="3c37d-450">Exercise 3</span></span>

<span data-ttu-id="3c37d-451">Подавать звуковой сигнал при распознавании объекта.</span><span class="sxs-lookup"><span data-stu-id="3c37d-451">Play a sound when an object is recognized.</span></span>

### <a name="exercise-4"></a><span data-ttu-id="3c37d-452">Упражнение 4</span><span class="sxs-lookup"><span data-stu-id="3c37d-452">Exercise 4</span></span>

<span data-ttu-id="3c37d-453">Используйте API для переобучения службы с теми же изображениями, которые анализирует приложение, чтобы сделать службу более точной (одновременное прогнозирование и обучение).</span><span class="sxs-lookup"><span data-stu-id="3c37d-453">Use the API to re-train your Service with the same images your app is analyzing, so to make the Service more accurate (do both prediction and training simultaneously).</span></span>
