---
title: 313. HoloLens (1-го поколения) и Azure — служба Центра Интернета вещей
...
ms.locfileid: "115195319"
---
# <a name="hololens-1st-gen-and-azure-313-iot-hub-service"></a>HoloLens (1 общий) и Azure 313: служба центра интернета вещей
...
- **azure приложение-функция** — это Microsoft Azure служба, которая позволяет разработчикам запускать в Azure небольшие фрагменты кода "функции". Это позволяет делегировать работу в облако, а не локальное приложение, которое может иметь множество преимуществ. **Функции Azure** поддерживают несколько языков разработки, включая C \# , F \# , Node.js, Java и PHP. Дополнительные сведения см. на странице [ **функций Azure**](/azure/azure-functions/functions-overview).

- **служба хранилища Azure: таблицы** — это Microsoft Azure служба, которая позволяет разработчикам хранить структурированные, не связанные с SQL, данные в облаке, что упрощает доступ к ним из любого места. Служба может похвастаться проект без схемы, позволяя развитию таблиц по мере необходимости и, таким образом, очень гибкой. Дополнительные сведения см. на странице [ **таблиц Azure** .](/azure/cosmos-db/table-storage-overview)
...
наиболее актуальные предварительные требования для разработки с использованием смешанной реальности, включая Microsoft HoloLens, см. в статье [установка средств](/windows/mixed-reality/install-the-tools) .
...
- Windows 10 Fall Creators Update (или более поздней версии), **режим разработчика включен**

    > [!WARNING]
    > вы не можете запустить виртуальную машину с помощью Hyper-V в Windows 10 Домашняя Edition.

- Windows 10 Пакет SDK (последняя версия)
- HoloLens, **режим разработчика включен**
...
- программное обеспечение **Hyper-V** , включенное на компьютере Windows 10ной разработки.
- Виртуальная машина под управлением Ubuntu (16,4 или 18,4), выполняемая на компьютере разработки, или можно использовать отдельный компьютер под управлением Linux (Ubuntu 16,4 или 18,4). дополнительные сведения о создании виртуальной машины на Windows с помощью Hyper-V см. в [разделе «перед](#before-you-start)началом работы». (https://docs.microsoft.com/virtualization/hyper-v-on-windows/quick-start/quick-create-virtual-machine).  



### <a name="before-you-start"></a>Перед началом работы

1. Настройте и протестируйте HoloLens. если вам требуется поддержка настройки HoloLens, [обязательно ознакомьтесь со статьей HoloLens установки](/hololens/hololens-setup).
2. рекомендуется выполнять настройку **калибровки** и **датчика** при начале разработки нового приложения HoloLens (иногда это может помочь в выполнении этих задач для каждого пользователя).
...
3. Настройте **виртуальную машину Ubuntu** с помощью **Hyper-V**. Следующие ресурсы помогут вам в процессе.
    ...
    2.  убедитесь, что на Windows 10 компьютере включен **Hyper-V** . Вы можете перейти по этой ссылке, чтобы получить рекомендации по [установке и включению Hyper-V на Windows 10](/virtualization/hyper-v-on-windows/quick-start/enable-hyper-v).
    3.  Запустите Hyper-V и создайте новую виртуальную машину Ubuntu. Вы можете перейти по этой ссылке, чтобы получить пошаговое [руководство по созданию виртуальной машины с помощью Hyper-V](/virtualization/hyper-v-on-windows/quick-start/create-virtual-machine). При запросе на **установку операционной системы из файла загрузочного образа** выберите ранее ЗАГРУЖЕНный ISO-образ **Ubuntu** .
...
1. в **Пользовательское визуальное распознавание Project** перейдите на вкладку **Performance (производительность** ).
...
8. запишите (вы можете использовать Блокнот) следующих параметров:
    1. **Сервер входа**
    2. **Имя пользователя**
    3. **Пароль**
...
3.  на новой странице будет представлено описание службы **учетных записей служба хранилища** . В нижнем левом углу этого запроса нажмите кнопку **создать** , чтобы создать экземпляр этой службы.
...
4.  После нажатия кнопки **создать** появится панель:
    ...
    1. Выберите **группу ресурсов** или создайте новую. Группа ресурсов предоставляет способ мониторинга, контроля доступа, подготовки счетов и управления ими для коллекции ресурсов Azure. Рекомендуется размещать все службы Azure, связанные с одним проектом (например, такие курсы), в общей группе ресурсов.
        ...
        > Если вы хотите ознакомиться с дополнительными сведениями о группах ресурсов Azure, перейдите [по этой ссылке, чтобы управлять группой ресурсов](/azure/azure-resource-manager/resource-group-portal).
...
17. запишите (на Блокнот) **строку подключения** (первичный ключ) для последующего использования при настройке *строки подключения* для устройства.
...
Чтобы создать и развернуть модули для *центра Интернета вещей*, вам потребуется установить следующие компоненты на компьютере разработки с Windows 10:
...
5. на этом этапе вам будет предложено открыть *файл конфигурации среды выполнения*, чтобы вставить **строку подключения устройства**, которая была записана (в Блокнот) при создании **службы центра интернета вещей** (в [шаге 14 раздела 3](#chapter-3---the-iot-hub-service)). Чтобы открыть этот файл, выполните в терминале следующую строку:
...
> [!IMPORTANT]
> следующие несколько глав (6-9) необходимо выполнить на компьютере Windows 10.
...
6. Теперь будет установлен **Cookiecutter**. В окне терминала выполните следующую команду Bash:
    ...
    >3. Убедитесь, что путь установки **Python** добавлен в качестве **переменной среды** на компьютере. Cookiecutter должен быть частью того же пути расположения. Чтобы получить [Дополнительные сведения о переменных среды](/windows/win32/procthread/environment-variables), перейдите по этой ссылке. 
...
6. Присвойте имя модулю. Нажмите клавишу **Ввод** , чтобы подтвердить имя модуля. обязательно запишите имя модуля (Блокнот), так как оно будет использовано позже.
...
вернитесь на портал azure, где вы создадите службу таблиц Azure, создав служба хранилища ресурс.

1. Если вы еще не вошли в систему, войдите на [портал Azure](https://portal.azure.com).

2. после входа в систему щелкните **создать ресурс** в верхнем левом углу и найдите **учетную запись служба хранилища** и нажмите клавишу **ввод** , чтобы начать поиск.

3. после появления щелкните **служба хранилища учетная запись — blob-объект, файл, таблица, очередь** из списка.

    ![Поиск учетной записи хранения](images/AzureLabs-Lab313-35.png)

4. на новой странице будет представлено описание службы **учетных записей служба хранилища** . В нижнем левом углу этого запроса нажмите кнопку **создать** , чтобы создать экземпляр этой службы.
...
5. После нажатия кнопки **создать** появится панель:
    ...
    3. для **типа учетной записи** в раскрывающемся меню выберите **служба хранилища (общего назначения v1)**.
    ...
    11. Нажмите кнопку **Create** (Создать).
...
8. нажмите кнопку " **переход к ресурсу** " в уведомлении, и вы будете перенаправлены на страницу обзора нового экземпляра службы служба хранилища.
...
13. теперь щелкните **ключи доступа** и создайте копию имени и **ключа** **учетной записи служба хранилища** (с помощью Блокнот) эти значения будут использоваться далее в этом курсе при создании **приложение-функция Azure**.

    ![создана новая таблица](images/AzureLabs-Lab313-43.png) 

14. снова с помощью панели слева перейдите к разделу *служба таблиц* и щелкните **таблицы** (или **обзор таблиц** в новых порталах) и скопируйте **URL-адрес таблицы** (с помощью Блокнот). это значение будет использоваться далее в этом курсе при связывании таблицы с приложением **Power BI** .
...
3. **Cloud Explorer** откроется как закрепленный элемент (подождите, так как загрузка может занять некоторое время).
    ...
    > [!WARNING] 
    > если подписка, использованная для создания *учетных записей служба хранилища* , не отображается, убедитесь, что у вас есть: 
...
4. Будут показаны облачные службы Azure. найдите **учетные записи служба хранилища** и щелкните стрелку слева от нее, чтобы развернуть учетные записи.

    ![Открытие учетных записей хранения](images/AzureLabs-Lab313-47.png)

5. после развертывания вновь созданная **учетная запись служба хранилища** должна быть доступна. Щелкните стрелку слева от хранилища, а затем после разворачивания найдите **таблицы** и щелкните стрелку рядом с ней, чтобы открыть **таблицу** , созданную в последней главе. Дважды щелкните **таблицу**.
...
1.  откройте **Блокнот** (нажмите *Windows клавишу* и введите *блокнот*).

    ![открыть Блокнот](images/AzureLabs-Lab313-51.png)

2.  после открытия Блокнот вставьте приведенную ниже структуру JSON. После этого сохраните его на рабочем столе, как **project.js**. Этот файл определяет библиотеки, которые будет использовать функция. Если вы использовали NuGet, он покажется знакомым.
    
    > [!WARNING]
    > Важно, чтобы именование было правильным; Убедитесь, что у него **нет .txt** расширения файла. Ниже приведены справочные сведения.
    >
...
6.  После нажатия кнопки **создать** заполните следующие поля:
    ...
    8. в разделе **служба хранилища** **необходимо выбрать службу служба хранилища, созданную на предыдущем шаге**.
...
19. измените следующие переменные, чтобы они соответствовали соответствующим значениям (**таблицы** и **служба хранилища** значения, из [шага 11 и 13, соответственно, в главе 11](#chapter-11---create-table-service)), которые находятся в **учетной записи служба хранилища**:

    - **tableName** с именем **таблицы** , расположенной в **учетной записи служба хранилища**.
    - **таблеурл** с URL-адресом **таблицы** , расположенной в **учетной записи служба хранилища**.
    - **storageAccountName** с именем значения, соответствующего имени **учетной записи служба хранилища** .
    - **storageAccountKey** с ключом, полученным в ранее созданной службе служба хранилища.
...
22. Панель будет продвигаться справа. на этой панели щелкните **Upload** и отобразится *обозреватель файлов* .

23. перейдите к и щелкните **project.js** файл, созданный в **Блокнот** ранее, а затем нажмите кнопку **открыть** . Этот файл определяет библиотеки, которые будет использовать функция.
...
если теперь открыть Visual Studio (**не** Visual Studio Code), можно визуализировать результат тестового сообщения, так как он будет храниться в области строк *мессажеконтент* .
...
для визуализации данных с вашего устройства интернета вещей, которое будет настроено **Power BI** (версия для настольной системы), можно получить данные из только что созданной службы *таблиц* . *HoloLensная* версия Power BI будет использовать эти данные для визуализации результата.
...
5.  щелкните **azure**, **служба хранилища таблиц azure**, а затем щелкните **Подключение**.
...
7.  при создании служба хранилища таблицы вам будет предложено вставить **служба хранилищаный ключ** (см.[шаг 11 главы 11](#chapter-11---create-table-service)). затем щелкните **Подключение**.
...
> [!WARNING]
> в следующей главе HoloLens конкретные. Power BI в настоящее время недоступна в качестве иммерсивного приложения, но вы можете запустить версию рабочего стола на портале Windows Mixed Reality (клифф House) через классическое приложение.

## <a name="chapter-16---display-power-bi-data-on-hololens"></a>глава 16-отображение Power BI данных на HoloLens

1. на HoloLens войдите в **Microsoft Store**, коснувшись значка в списке приложений.
...
Поздравляем! вы успешно создали службу центра Интернета вещей с имитацией устройства с виртуальными машинами. устройство может передавать результаты модели машинного обучения в службу таблиц azure, что облегчается приложение-функция Azure, которые считываются в Power BI и выводятся на визуализацию в Microsoft HoloLens.
