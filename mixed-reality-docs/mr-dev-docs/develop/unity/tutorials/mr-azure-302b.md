---
title: 302b. HoloLens (1-го поколения) и Azure — пользовательское визуальное распознавание
...
ms.locfileid: "115217743"
---
# <a name="hololens-1st-gen-and-azure-302b-custom-vision"></a>HoloLens (1 общий) и Azure 302b: пользовательское представление
...
Эта служба позволит обучить модель машинного обучения с помощью образов объектов. затем обученная модель будет использоваться для распознавания похожих объектов, как это обеспечивается захватом камеры Microsoft HoloLens или камерой, подключенной к компьютеру для использования в качестве впечатляющих (VR) гарнитур.
...
-   **Режим анализа**. Настройка пользовательская служба визуального распознавания вручную путем отправки образов, создания тегов и обучения службы для распознавания различных объектов (в данном случае это мышь и клавиатура). затем вы создадите HoloLens приложение, которое будет записывать изображения с помощью камеры и попытается распознать эти объекты в реальном мире.
...
5.  Принимаю условия, вы будете переходить к разделу *проекты* на портале. Щелкните **New Project (создать**).
...
6.  На правой стороне появится вкладка, в которой будет предложено указать некоторые поля для проекта.
    ...
    4. задание *типов Project* для **классификации**
...
7.  щелкните **Upload файлы**. Файлы начнут отправляться. Получив подтверждение отправки, нажмите кнопку **Готово**.
...
14. также скопируйте **идентификатор Project**, а также вставьте его в файл *Блокнот* для последующего использования.
...
8.  назад в *сборке Параметры* *\# проекты Unity C* больше не выделяются. установите флажок рядом с этим.
...
1.  щелкните правой кнопкой мыши *папку Asset* , расположенную на *панели Project*, а затем выберите пункт **создать > папку**. Вызовите **скрипты** папки.
...
6.  В классе *кустомвисионаналисер* добавьте следующие переменные:
    ...
    > [!NOTE]
    > Убедитесь, что вы вставили **ключ прогноза** в переменную **Предиктионкэй** и свою **конечную точку прогнозирования** в переменную **предиктионендпоинт** . вы скопировали их для *Блокнот* ранее в этом курсе.
...
2.  Дважды щелкните новый скрипт **воицерекогнизер** , чтобы открыть его с помощью **Visual Studio**.
...
2.  Дважды щелкните новый скрипт *кустомвисионтраинер* , чтобы открыть его с помощью **Visual Studio**.
...
4.  Затем добавьте следующие переменные в класс *кустомвисионтраинер* над методом **Start ()** . 
    ...
    > [!IMPORTANT]
    > убедитесь, что вы добавили **ключ службы** (ключ обучения) и значение **идентификатора Project** , которое вы заговорили ранее. Это значения, [собранные на портале ранее в ходе курса (глава 2, шаг 10 и выше)](#chapter-2---training-your-custom-vision-project).
...
- вы вставили **идентификатор Project** в переменную **projectId** класса *кустомвисионтраинер* .
...
4.  На *платформе решения* выберите **x86, удаленный компьютер**. вам будет предложено вставить **IP-адрес** удаленного устройства (в данном случае HoloLens, который вы заметили).
