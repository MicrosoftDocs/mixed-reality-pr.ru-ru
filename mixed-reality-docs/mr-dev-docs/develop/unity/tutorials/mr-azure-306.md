---
title: 306. HoloLens (1-го поколения) и Azure — потоковая передача видео
...
ms.locfileid: "115224377"
---
# <a name="hololens-1st-gen-and-azure-306-streaming-video"></a>HoloLens (1 gen) и Azure 306: потоковая передача видео
...
в этом курсе вы узнаете, как подключить Службы мультимедиа Azure к Windows Mixed Reality VR, чтобы разрешить воспроизведение видео в режиме потоковой передачи 360 на впечатляющих наушниках. 

**Службы мультимедиа Azure** — это набор служб, которые предоставляют услуги потоковой передачи видео с передачей данных для достижения больших аудиторий на современных наиболее популярных мобильных устройствах. дополнительные сведения см. на [странице Службы мультимедиа Azure](https://azure.microsoft.com/services/media-services).

Прополнив этот курс, вы получите иммерсивное приложение для наушников, которое сможет сделать следующее:

1. получите видео в 360 градусов от **служба хранилища Azure** с помощью **службы мультимедиа Azure**.
...
- [головной элемент «Windows Mixed Reality иммерсивное (VR)](../../../discover/immersive-headset-hardware-details.md) »
...
2.  Настройка и тестирование иммерсивного наушников смешанной реальности.
    ...
    > [!NOTE]
    > Для этого курса **не** потребуется использовать контроллеры движения. Если вам нужна поддержка настройки иммерсивного гарнитуры, щелкните [ссылку, чтобы настроить Windows Mixed Reality](https://support.microsoft.com/help/4043101/windows-10-set-up-windows-mixed-reality).

## <a name="chapter-1---the-azure-portal-creating-the-azure-storage-account"></a>глава 1. создание учетной записи служба хранилища Azure на портале Azure
...
2.  после входа в меню слева щелкните **учетные записи служба хранилища** .
    ...
    ![служба хранилища Azure Настройка учетной записи](images/AzureLabs-Lab6-02.png)

3.  на вкладке **учетные записи служба хранилища** щелкните **добавить**.

    ![служба хранилища Azure Настройка учетной записи](images/AzureLabs-Lab6-03.png)
...
5.  Необходимо подтвердить, что вы поняли условия, примененные к этой службе.
    ...
    ![служба хранилища Azure Настройка учетной записи](images/AzureLabs-Lab6-04.png)
...
7.  После создания экземпляра службы на портале отобразится уведомление.
    ...
    ![служба хранилища Azure Настройка учетной записи](images/AzureLabs-Lab6-05.png)
...
3.  После нажатия кнопки **создать** панель появится место, где необходимо указать сведения о новой службе мультимедиа.
    ...
    5.  в разделе **учетная запись служба хранилища** щелкните раздел **выбрать...** , а затем выберите **учетную запись служба хранилища** , созданную в последней главе.
...
9.  На следующей странице в левом верхнем углу страницы щелкните **upload**.
...
23. Вам будет предложено задать **указатель**, который является точкой входа, к файлу/s в ваших ресурсах. Для сценария задайте следующие свойства:
    ...
    > [!NOTE]
    > дополнительные сведения об указателях и о том, что можно выбрать, см. в [документации по Службы мультимедиа Azure](/azure/media-services/media-services-concepts).
...
## <a name="chapter-3---setting-up-the-unity-project"></a>Глава 3. Настройка Unity Project
...
2.  теперь необходимо указать имя Project Unity, вставить **MR \_ 360VideoStreaming.**. Убедитесь, что для типа проекта задано значение **3D**. Задайте для расположения нужное расположение (Помните, что ближе к корневым каталогам лучше). Затем нажмите кнопку **создать проект**.

    ![портал Azure;](images/AzureLabs-Lab6-29.png)

3.  При открытом Unity стоит проверить, что **Редактор скриптов** по умолчанию имеет значение **Visual Studio.** Перейдите к разделу **_изменение_ *настроек*** , а затем в новом окне перейдите к разделу **Внешние инструменты**. измените **редактор внешних скриптов** на **Visual Studio 2017**. Закройте окно **настройки** .
...
5.  Также убедитесь, что:
    ...
    7.  Остальные параметры должны остаться в данный момент по умолчанию.
        ...
        ![Настройка Project Unity](images/AzureLabs-Lab6-31.png)
...
7. На этой панели необходимо проверить несколько параметров:
    ...
    1.  на вкладке **другие Параметры** :
        ...
        3. **Уровень совместимости API** должен быть **.NET 4,6.**
            ...
            ![Настройка Project Unity](images/AzureLabs-Lab6-32.png)
    ...
    2.  далее на панели в **XR Параметры** (находится под **Параметры публикации**), **поддерживаемая виртуальная реальность** tick, убедитесь, что добавлен **пакет SDK Windows Mixed Reality** .
        ...
        ![Настройка Project Unity](images/AzureLabs-Lab6-33.png)

    3.  на вкладке **публикация Параметры** в разделе **возможности** установите флажок:

        - **InternetClient**;

            ![Настройка Project Unity](images/AzureLabs-Lab6-34.png)

8.  после внесения этих изменений закройте окно **сборка Параметры** .

9.  сохраните Project **файл* * сохранить Project * *.
...
1.  щелкните правой кнопкой мыши **папку Asset**, расположенную на панели **Project** , и выберите пункт **создать > папку**. Назовите папку **Scripts**.
...
4.  дважды щелкните новый скрипт **видеоконтроллер** , чтобы открыть его с помощью **Visual Studio 2017.**
...
2.  щелкните правой кнопкой мыши панель **Project** , **создать* * C \# Script * *. Присвойте скрипту имя « **взгляд**».

3.  дважды щелкните новый скрипт ***взгляните**, чтобы открыть его с помощью _ *Visual Studio 2017.**
...
7.  в двух сценах перейдите в раздел **файл > сборка Параметры**. после открытия окна **Параметры сборки** перетащите фоновый кадр в раздел **сцены в сборке** .
...
15. перейдите к папке **materials** на **панели Project**. Дублировать материал **буттонматериал** (нажмите клавиши **CTRL**  +  **D** на клавиатуре или щелкните его левой кнопкой мыши , а затем в меню **изменить** файл выберите пункт **дублировать**).
...
3.  последовательно выберите **файл > сборка Параметры** и щелкните **сборка**.
...
После завершения сборки в расположение сборки появится окно **проводника файлов** . откройте папку с именем и построением, а затем дважды щелкните файл решения (sln) в этой папке, чтобы открыть решение с Visual Studio 2017.
...
4.  Теперь необходимо восстановить все пакеты в решении. щелкните **решение** правой кнопкой мыши и выберите пункт **восстановить NuGet пакеты для решения...**
...
5.  Перейдите в **меню "сборка** " и щелкните " **Развернуть решение** ", чтобы загружать неопубликованные приложение на компьютере. Visual Studio сначала выполнит сборку и развертывание приложения.
