---
title: 301. HoloLens (1-го поколения) и Azure — перевод с одного языка на другой
...
ms.locfileid: "115229750"
---
# <a name="hololens-1st-gen-and-azure-301-language-translation"></a>HoloLens (1 gen) и Azure 301: языковой перевод
...
в этом курсе вы узнаете, как получить результаты из службы Переводчик в образце приложения на основе Unity. Вы сможете применить эти понятия к настраиваемому приложению, которое вы можете собрать.
...
- чтобы разрешить компьютеру включить диктовку, перейдите в раздел **Параметры > конфиденциальность > речи, рукописный ввод & ввода** и нажмите кнопку **включить речевые службы и ввести предложения**.
- если вы используете микрофон и наушники, подключенные к гарнитуре (или встроенные в), убедитесь, что параметр "при износе гарнитуры, переключиться на микрофон гарнитуры" включен в **Параметры > смешанной реальности > аудио и речи**.
...
8.  Нажмите кнопку " **Переход к ресурсу** " в уведомлении, чтобы изучить новый экземпляр службы. вы будете перенаправлены на новый экземпляр службы API перевода текстов. 
    ...
    ![Переводчик Страница службы текстового API](images/AzureLabs-Lab1-07.png)
...
2.  теперь необходимо указать имя Project Unity. Вставка **MR_Translation**. Убедитесь, что для типа проекта задано значение **3D**. Задайте для *расположения нужное расположение* (Помните, что ближе к корневым каталогам лучше). Затем нажмите кнопку **создать проект**.
...
4.  затем перейдите в раздел **файл > сборка Параметры** и переключите платформу на **универсальная платформа Windows**, нажав кнопку **коммутатора платформы** .
    ...
    ![создание Параметры окна, переключение платформы в UWP.](images/AzureLabs-Lab1-11.png)
...
3.  Дважды щелкните новый скрипт *Results* , чтобы открыть его с помощью **Visual Studio**.
...
- после приостановки голоса отправьте диктовку в класс Переводчик.
...
8.  теперь вам потребуются методы, используемые приложением для запуска и завершения записи речи, и передайте его в класс *Переводчик* , который скоро будет создан. Скопируйте приведенный ниже код и вставьте его под методом *Start ()* .
...
9.  Теперь необходимо добавить обработчик диктовки, который будет вызываться при остановке голоса. затем этот метод передает продиктованный текст в класс *Переводчик* .
...
> [!WARNING]  
> на этом этапе вы увидите ошибку на панели *консоли редактора Unity* ("имя" Переводчик "не существует..."). это обусловлено тем, что код ссылается на класс *Переводчик* , который будет создан в следующей главе.

## <a name="chapter-7--call-to-azure-and-translator-service"></a>Глава 7 — вызов службы Azure и переводчика

последний скрипт, который необходимо создать, — это *Переводчик* класс. 
...
2.  щелкните правой кнопкой мыши на **панели Project**, **создайте > сценарий C#**. вызовите *Переводчик* скрипта.
3.  дважды щелкните новый скрипт *Переводчик* , чтобы открыть его **с Visual Studio**.
...
5.  затем добавьте следующие переменные в класс *Переводчик* :
...
4.  затем перетащите класс *Переводчик* из папки **scripts** в **главный объект Camera** на *панели иерархия*. 
...
- *результаты*, *Переводчик* и *микрофонеманажер*, скрипты присоединяются к **главному объекту Camera** . 
- вы поместили **ключ** службы *API перевода текстов Azure* в переменную **authorizationkey согласно инструкциям** в скрипте *Переводчик* .  
