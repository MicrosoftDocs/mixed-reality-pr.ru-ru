...
---
# <a name="mr-and-azure-307-machine-learning"></a>307. Смешанная реальность и Azure: машинное обучение
...
8.  В **_нижнем левом_ углу элемента *_* Split элемент данных** перетащите соединение в **верхнюю правую** часть элемента **обучение модели** . В модели обучения для обучения алгоритму будет использоваться первый 70%, разделенный на набор данных.
...
14. В списке элементов эксперимента на панели слева разверните узел **Machine Learning**  >  **Оценка** машинное обучение и перетащите элемент **модель оценки** на холст.
...
3.  Затем перейдите в раздел **File**  >  **параметры сборки** файлов и переключайте платформу на **универсальная платформа Windows**, нажав кнопку **_Сменить платформу_* _.

4.  Также убедитесь, что:

    1.  Для параметра _ *Target Device** задано **любое устройство**.
...
4.  Вернувшись в редактор Unity, убедитесь, что класс **шелфкипер** выглядит следующим образом:
    ...
    > 4. Перетащите **точки порождения** с **панели Иерархия** (под объектом *полка* ) на 3 ссылки на **три** **элемента** , как **Spawn Point** показано на рисунке.
    > 
...
15. Сохраните сцену и файл проекта **File**  >  **сохранить сцену/файл**  >  **сохранить проект**.
...
2.  К **File**  >  **параметрам сборки** файлов
