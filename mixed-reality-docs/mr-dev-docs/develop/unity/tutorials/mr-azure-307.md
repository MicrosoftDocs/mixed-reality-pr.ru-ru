...
ms.locfileid: "115199177"
---
# <a name="hololens-1st-gen-and-azure-307-machine-learning"></a>HoloLens (1 gen) и Azure 307: машинное обучение
...
2.  создайте **Project Unity**, который может принимать и интерпретировать данные прогноза из службы ML.
3.  визуально отображать данные затенения в **Project Unity**, предоставляя наиболее популярные элементы продаж на полке.
...
- доступ к интернету для установки Azure и ML получение данных
...
## <a name="chapter-1---azure-storage-account-setup"></a>глава 1. настройка учетной записи служба хранилища Azure

чтобы использовать Azure Переводчик API, необходимо настроить экземпляр службы, чтобы сделать ее доступной для приложения.
...
2.  после входа в меню слева щелкните **учетные записи служба хранилища** .

    ![служба хранилища Azure Настройка учетной записи](images/AzureLabs-Lab7-1.png)

    > [!NOTE]
    > Слово **New** может быть заменено на **создать ресурс** в новых порталах.

3.  на вкладке **учетные записи служба хранилища** щелкните **добавить**.

    ![служба хранилища Azure Настройка учетной записи](images/AzureLabs-Lab7-2.png)

4.  на панели **создание учетной записи служба хранилища** :
    ...
    3.  в качестве **типа учетной записи** выберите **служба хранилища (общего назначения** версии 1).
...
5.  Также необходимо подтвердить, что вы поняли условия, примененные к этой службе.
    ...
    ![служба хранилища Azure Настройка учетной записи](images/AzureLabs-Lab7-3.png)
...
7.  После создания экземпляра службы на портале отобразится уведомление.
    ...
    ![служба хранилища Azure Настройка учетной записи](images/AzureLabs-Lab7-4.png)
...
3.  после нажатия кнопки **создать** появится панель, где необходимо указать некоторые сведения о новой **службе Машинное обучение Studio**:
    ...
    4.  Определите **Расположение** группы ресурсов (при создании новой группы ресурсов). В идеале это расположение будет находиться в регионе, в котором будет выполняться приложение. Некоторые ресурсы Azure доступны только в определенных регионах. следует использовать ту же группу ресурсов, которая использовалась для создания служба хранилища Azure в предыдущей главе.

    5.  в разделе **учетная запись служба хранилища** щелкните **использовать существующий**, а затем в раскрывающемся меню выберите **учетную запись служба хранилища** , созданную в последней главе.
...
> [!IMPORTANT]
> Приведенный выше файл .zip содержит и **продуктстаблексв** , и **пакет unitypackage**, которые понадобятся в [главе 6](#chapter-6---importing-the-mlproducts-unity-package). Этот пакет также предоставляется в этой главе, но разделяются на CSV-файл.
...
1.  вернитесь на портал **Машинное обучение Studio (классический)** и добавьте эту таблицу в качестве **набора данных** для ML. Для этого нажмите кнопку **+ создать** в левом нижнем углу экрана.
...
3.  Upload новый **набор данных** , выполнив следующие действия.
    ...
    4.  В раскрывающемся меню **тип** выберите **универсальный CSV-файл с заголовком (.csv)**.
...
13. Подключение выходные данные из нижней части **логистической регрессии многоклассового класса** в верхний левый вход элемента **обучение модели** .
...
15. Подключение выходные данные из нижней части **модели обучения** в верхний левый вход **модели оценки**.

16. Подключение нижний правый вывод из раздела « **разбиение данных**» к верхнему правому входу элемента « **оценка модели** ».
...
18. Подключение выходные данные **модели оценки** к верхнему левому входу **модели Evaluate**.
...
## <a name="chapter-5---setting-up-the-unity-project"></a>Глава 5. Настройка Unity Project
...
1.  откройте **unity** и создайте новый Project unity с именем **MR \_ MachineLearning.** Убедитесь, что для типа проекта задано значение **3D**.

2.  При открытом Unity стоит проверить, что **Редактор скриптов** по умолчанию имеет значение **Visual Studio**. Перейдите к разделу **изменение**  >  **настроек** , а затем в новом окне перейдите к разделу **Внешние инструменты**. измените **редактор внешних скриптов** на **Visual Studio 2017**. Закройте окно **настройки** .

3.  затем перейдите в раздел   >  **сборка файла Параметры** и переключите платформу на **универсальная платформа Windows**, нажав кнопку **_коммутатора платформы_** .
...
4.  Также убедитесь, что:
    ...
    7.  Остальные параметры должны остаться в данный момент по умолчанию.
        ...
        ![Настройка Project Unity](images/AzureLabs-Lab7-35.png)

5.  в окне **сборка Параметры** нажмите кнопку **Параметры проигрывателя** , чтобы открыть связанную панель в пространстве, где находится **инспектор** . 
...
6. На этой панели необходимо проверить несколько параметров:
    ...
    1.  на вкладке **другие Параметры** :
        ...
        3. **Уровень совместимости API** должен быть **.NET 4,6**
            ...
            ![Настройка Project Unity](images/AzureLabs-Lab7-36.png)

    2.  на вкладке **публикация Параметры** в разделе **возможности** установите флажок:

        - **InternetClient**;

            ![Настройка Project Unity](images/AzureLabs-Lab7-37.png)
    ...
    3.  далее на панели в **XR Параметры** (находится под **Параметры публикации**), **поддерживаемая виртуальная реальность** tick, убедитесь, что добавлен **пакет SDK Windows Mixed Reality**
        ...
        ![Настройка Project Unity](images/AzureLabs-Lab7-38.png)

    

6.  назад в **сборке Параметры** проекты *C# для Unity* больше не заменяются. Установите флажок рядом с этим. 

7.  Закройте окно Build Settings (Параметры сборки).

8.  сохраните Project (**файл > сохранить проект**).
...
4.  после завершения импорта вы увидите, что некоторые новые папки появились на **панели Project** Unity. Это трехмерные модели и соответствующие материалы, которые являются частью предварительно подготовленной сцены, с которой вы будете работать. В этом курсе будет написана большая часть кода.

    ![Импорт пакета Unity Млпродуктс](images/AzureLabs-Lab7-41.png)

5.  в папке **Project панель** щелкните папку « **сцены** » и дважды щелкните сцену внутри (под названием **MR_MachineLearningScene**). Будет открыта сцена (см. изображение ниже). Если красные ромбы отсутствуют, просто нажмите кнопку **приспособлений** в правом верхнем углу **игровой панели**.
...
1.  дважды щелкните сценарий **шелфкипер** в папке **scripts** , чтобы открыть его с помощью **Visual Studio 2017**.
...
1.  перейдите в папку **scripts** на **панели Project**.

2.  Щелкните правой кнопкой мыши внутри папки и выберите **создать**  >  **скрипт C#**. Вызовите скрипт **продуктпредиктион**.

3.  дважды щелкните новый скрипт **продуктпредиктион** , чтобы открыть его с помощью **Visual Studio 2017**.
...
9.  ниже приведен метод, собирающий дату и время из Windows и преобразующий их в формат, который эксперимент Машинное обучение может использовать для сравнения с данными, хранящимися в таблице.
...
13. сохраните **Visual Studio** и отвернемся к **Unity**.

14. Перетащите скрипт класса **продуктпредиктион** из папки **script** на **основной объект Camera** .

15. сохраните сцену и файл проекта   >  **сохранить сцену/файл**  >  **сохранить Project**.
...
2.  последовательно выберите **файл**  >  **сборка Параметры**
...
1.  Перейдите к новой сборке Unity (папка **приложения** ) и откройте файл решения с помощью **Visual Studio**.

2.  после открытия Visual Studio необходимо восстановить пакеты NuGet, которые можно выполнить, щелкнув решение MachineLearningLab_Build правой кнопкой мыши, из обозреватель решений (справа от Visual Studio) и выбрав пункт восстановить NuGet пакеты:
...
4.  На платформе решения выберите **x86**, **локальный компьютер**. 
    ...
    > - определите **IP-адрес** HoloLens, который можно найти в *Параметры > Network & Internet > Wi-Fi > дополнительных параметров*. IPv4 — это адрес, который следует использовать. 
    > - Убедитесь, **что включен** **режим разработчика** ; находится в *Параметры > Update & > безопасности для разработчиков*.
