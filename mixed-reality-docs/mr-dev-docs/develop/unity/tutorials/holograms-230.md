...
* проверьте окружение и передайте данные из HoloLens на компьютер разработки.
...
>[!NOTE]
>Если вы хотите просмотреть исходный код перед загрузкой, он будет [доступен на GitHub](https://github.com/Microsoft/HolographicAcademy/tree/Holograms-230-SpatialMapping).

### <a name="notes"></a>Примечания

* параметр "включить Только мой код" в Visual Studio должен быть отключен (*снят*) в меню сервис > параметры > отладка для попадания в код точек останова в коде.
...
* после запуска Unity перейдите в раздел **изменение > Project Параметры > Player**.
* на панели **инспектора** найдите и выберите зеленый значок **хранилища Windows** .
* разверните **другие Параметры**.
* В разделе " **Подготовка к просмотру** " установите флажок **поддерживаемый виртуальный реальность** .
* убедитесь, что в списке **пакетов sdk виртуальной реальности** отображается **Windows holographic** . в противном случае нажмите **+** кнопку в нижней части списка и выберите **Windows holographic**.
* разверните **Параметры публикации**.
* В разделе **возможности** проверьте следующие параметры.
    * InternetClientServer;
    * PrivateNetworkClientServer;
    * Микрофон
    * SpatialPerception;
* перейдите к разделу **изменение > Project Параметры > качество**
* на панели **инспектора** под значком Windows хранилище выберите черную стрелку раскрывающегося списка в строке "по умолчанию" и измените значение по умолчанию на **очень низкое**.
* Перейдите в раздел **активы > импортировать пакет > настраиваемый пакет**.
* перейдите в папку **. ..\холографикакадеми-Голограммы-230-спатиалмаппинг\стартинг** .
...
* в папке **Project** **HoloToolkit-SpatialMapping-230\Utilities\Prefabs** найдите **основной объект Camera** .
* Перетащите **основной prefab камеры** в панель **Иерархия** .
* На панели **Иерархия** удалите объект **направленного освещения** .
* на панели **Project** **Голограммы** папку, выберите объект **курсора** .
...
* в папке **Project** панель **HoloToolkit-SpatialMapping-230\SpatialMapping\Prefabs** найдите **спатиалмаппинг** prefab.
...
* в Visual Studio воспользуйтесь верхней панелью инструментов, чтобы изменить конфигурацию на **выпуск**.
...
* вернитесь к Visual Studio и удалите строку **трианглесперкубикметер** , добавленную в функцию " **спящий ()** " во время предыдущего раздела.
* Повторно разверните проект на устройстве. Теперь мы должны работать с **500** треугольниками на кубический метр.
* откройте браузер и введите HoloLens IPAddress, чтобы перейти на **портал устройств Windows**.
* Выберите параметр **объемное представление** на панели слева.
* В разделе **реконструкция поверхности** нажмите кнопку **Обновить** .
* посмотрите, как области, просмотренные на HoloLens, отображаются в окне отображения.
...
* в папке **Project** панели **шейдеров** дважды щелкните **блуелинесонваллс** , чтобы открыть шейдер в Visual Studio.
...
* на панели **Project** найдите папку **материалы** и выберите материал **блуелинесонваллс** .
...
* выполните сборку и развертывание в HoloLens и посмотрите, как отрисовка шейдера отображается на реальных поверхностях.
...
* в **Projectной** панели Unity **Голограммы** папку найдите объект **спатиалпроцессинг** .
* Перетащите & объект **спатиалпроцессинг** на панель **Иерархия** .

Спатиалпроцессинг prefab включает компоненты для обработки данных пространственного сопоставления. **Сурфацемешестопланес. CS** найдет и создаст плоскости на основе данных пространственного сопоставления. Для представления стен, этажей и цеилингс мы будем использовать плоскости в нашем приложении. Этот prefab также включает **ремовесурфацевертицес. CS** , который может удалять вершины из сетки пространственных сопоставлений. Это можно использовать для создания отверстий в сетке или удаления лишних треугольников, которые больше не нужны (так как вместо них можно использовать плоскости).

* в **Projectной** панели Unity **Голограммы** папку найдите объект **спацеколлектион** .
...
* Дважды щелкните **плайспацеманажер. CS** , чтобы открыть его в Visual Studio.

Плайспацеманажер. cs содержит код, зависящий от приложения. Мы добавим в этот скрипт функциональные возможности, чтобы обеспечить следующее поведение:
...
Завершите упражнения по написанию кода, отмеченные в Плайспацеманажер. cs, или замените скрипт завершенным решением ниже:
...
* перед развертыванием в HoloLens нажмите кнопку **воспроизвести** в Unity, чтобы войти в режим воспроизведения.
...
* на панели **Project** , в папке **scripts** дважды щелкните папку **. cs** , чтобы открыть ее в Visual Studio.
...
Раскомментируйте весь код в приведенном ниже упражнении кода или используйте это готовое решение **. CS**:
...
* на панели **Project** в папке **Голограммы** разверните объект **соларсистем** .
* Щелкните **Земля**.
...
Наконец, мы будем включать результат концепции x-ray для планет в нашей солнечной системе. Для достижения следующих целей потребуется изменить **планетокклусион. CS** (находится в папке скриптс\соларсистем).
...
Выполните упражнения по написанию кода в Планетокклусион. cs или используйте следующее решение:
...
* выполните сборку и развертывание приложения в HoloLens, как обычно.
