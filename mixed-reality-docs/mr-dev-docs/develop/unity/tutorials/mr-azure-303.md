---
title: HoloLens (1 gen) и Azure 303 — понимание естественного языка (LUIS)
...
ms.locfileid: "115218244"
---
# <a name="hololens-1st-gen-and-azure-303-natural-language-understanding-luis"></a>HoloLens (1-й общий) и Azure 303: понимание естественного языка (LUIS)
...
В этом курсе вы узнаете, как интегрировать Распознавание речи в приложение смешанной реальности с помощью Cognitive Services Azure с API распознавания речи.

![Результат лаборатории](images/AzureLabs-Lab3-000.png)

*Распознавание речи (LUIS)* — это Microsoft Azure служба, которая предоставляет приложениям возможность принимать значения из вводимых пользователем данных, например, путем извлечения того, что может потребоваться пользователю, в отдельных словах. Это достигается благодаря машинному обучению, которое понимает и изучает входные данные, а затем может ответить на подробные и релевантные сведения. Дополнительные сведения см. на [странице распознавание речи Azure (Luis)](https://azure.microsoft.com/services/cognitive-services/language-understanding-intelligent-service/).
...
2.  чтобы разрешить компьютеру включить диктовку, перейдите в **Windows Параметры > конфиденциальность > речи, ввод рукописного ввода & ввода** и нажмите кнопку " **включить службы речи" и введите рекомендации**.
...
Чтобы использовать службу *Распознавание речи* в Azure, необходимо настроить экземпляр службы, чтобы сделать ее доступной для приложения.
...
2.  Войдя в систему, щелкните New ( **создать** ) в левом верхнем углу, найдите *Распознавание речи* и нажмите клавишу **Ввод**. 
...
3.  На новой странице справа будет представлено описание службы Распознавание речи. В нижнем левом углу этой страницы нажмите кнопку **создать** , чтобы создать экземпляр этой службы.
...
12. На странице *Служба* щелкните *Распознавание речи портал* для перенаправления на веб-страницу, которая будет использоваться для создания новой службы в приложении Luis. 

## <a name="chapter-2--the-language-understanding-portal"></a>Глава 2 — портал Распознавание речи
...
1.  При достижении *Распознавание речи портала* вам может потребоваться выполнить вход, если вы еще не сделали этого, с теми же учетными данными, что и у портал Azure. 
...
2.  теперь необходимо указать имя Project Unity, вставить **MR_LUIS**. Убедитесь, что для типа проекта задано значение **3D**. Задайте для **расположения нужное расположение** (Помните, что ближе к корневым каталогам лучше). Затем нажмите кнопку **создать проект**.
...
4.  затем перейдите в раздел **файл > сборка Параметры** и переключите платформу на **универсальная платформа Windows**, нажав кнопку **коммутатора платформы** .
    ...
    ![создание Параметры окна, переключение платформы в UWP.](images/AzureLabs-Lab3-27.png)
...
1.  щелкните правой кнопкой мыши на *панели Project*, **создайте папку >**. Вызовите **скрипты** папки. 
...
3.  Дважды щелкните *микрофонеманажер* , чтобы открыть его с *Visual Studio*.
...
Поздравляем! вы создали приложение смешанной реальности, которое использует службу Azure Распознавание речи Intelligence, чтобы понять, что говорит пользователь, и действовать над этими сведениями.
