...
---
# <a name="hololens-1st-gen-and-azure-312-bot-integration"></a>HoloLens (1-й общий) и Azure 312: интеграция с Bot
...
1.  Запустите Visual Studio. Создайте новый проект, выберите в качестве типа проекта **веб-приложение ASP NET Core** (его можно найти в подразделе .NET Core) и вызовите его **мибот**. Нажмите кнопку **OK**.

2.  В окне, которое будет отображаться, выберите **пустое**. Также убедитесь, что для целевого объекта задано значение **ASP NET Core 2,0** , а для параметра Проверка подлинности — значение **без проверки подлинности**. Нажмите кнопку **OK**.  
...
5. На странице **Публикация** вам нужно будет задать ту же **подписку** , которая использовалась для создания службы *веб-приложения Bot* . Затем задайте **представление** в качестве **группы ресурсов** и в раскрывающейся структуре папок выберите **группу ресурсов** , созданную ранее. Нажмите кнопку **OK**. 
