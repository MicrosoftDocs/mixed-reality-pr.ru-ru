...
ms.date: 10/22/2019
ms.topic: article
keywords: холотулкит, микседреалититулкит, микседреалититулкит-unity, academy, учебник, взгляд, HoloLens, academy reality, unity, гарнитура смешанной реальности, гарнитура windows mixed reality, гарнитура виртуальной реальности, Windows 10
ms.openlocfilehash: 7961072eaabc04927f2b01e97f843ad60e4825e8
ms.sourcegitcommit: bea83261bf9ce7a27a618e5bc54dc4d7711f5435
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 10/19/2021
ms.locfileid: "130154085"
---
# <a name="hololens-1st-gen-input-210-gaze"></a>HoloLens (1-й общий) вход 210: взгляд
...
>[!NOTE]
>Если вы хотите просмотреть исходный код перед загрузкой, он будет [доступен на GitHub](https://github.com/Microsoft/HolographicAcademy/tree/Holograms-210-Gaze).

### <a name="errata-and-notes"></a>Ошибки и примечания

* в Visual Studio параметр "Только мой код" должен быть отключен (снят) в разделе сервис->параметры — >отладка для попадания в точки останова в коде.
...
6. Нажмите кнопку **создать Project**.
...
2. на **панели инспектора** для Параметры проигрывателя выберите значок **хранилище Windows** .
...
1. перейдите в раздел **изменение > Project Параметры > Player** (возможно, это можно было бы выполнить на предыдущем шаге).
2. на **панели инспектора** для Параметры проигрывателя выберите значок **хранилище Windows** .
3. в разделе настройка **другого Параметры** убедитесь, что для **серверной части скрипта** задано значение **.net** .

Наконец, мы будем обновлять наши параметры качества, чтобы обеспечить высокую производительность HoloLens.

1. выберите **изменить > Project Параметры > качества**.
2. щелкните стрелку вниз в строке **по умолчанию** под значком хранилища Windows.
3. выберите **очень низкое** для **приложений Windows Store**.
...
8. в папке **Голограммы** найдите ресурс **фитбокс** и перетащите его в корень **иерархии**.
...
1. перестройте приложение из **файла > сборки Параметры**.
...
2. Дважды щелкните **интерактибле. CS** , чтобы открыть его в Visual Studio.
3. Раскомментируйте строки в обратных вызовах **ифокусабле. онфокусентер ()** и **ифокусабле. онфокусексит ()** в **интерактибле. CS**. они вызываются InputManagerами смешанной набор средств реальности, когда фокус (либо с помощью указателя мыши, либо с помощью контроллера) вводит и выходит из GameObjectа.
...
2. На панели **инспектора** найдите скрипт **стабилизер** . щелкните его, чтобы открыть в Visual Studio, если хотите взглянуть на него.
...
Мы будем использовать файл **директиониндикатор. CS** , который будет выполнять следующие действия:
...
6. на панели **Project** в папке **Голограммы** перетащите ресурс **директионалиндикатор** в свойство « **указатель направления** » в **инспекторе**.
...
Мы будем использовать файл **объявления. CS** , чтобы поддерживать GameObject, чтобы он всегда был подключен к пользователю.
...
Мы будем использовать файл **симплетагалонг. CS** , который будет выполнять следующие действия:
...
Для этого сначала необходимо изменить сценарий **интерактибле. CS** для вызова **тагалонгактион**.

1. Измените **интерактибле. CS** , выполнив написание упражнения 6. a (Раскомментируйте строки с 84 по 87).
...
Сценарий **интерактиблеактион. CS** , состоящий из **интерактибле. CS** , выполняет пользовательские действия при касании голограмм. В этом случае мы будем использовать один из них специально для тега.

* В папке **Scripts** щелкните ресурс **тагалонгактион. CS** , чтобы открыть его в Visual Studio.
* Завершите упражнение по написанию кода или измените его на следующее:
  ...
  * в папке **Голограммы** найдите ресурс **тагалонг** .
  * Выберите объект **ChestButton_Center** в **иерархии**. перетащите объект **тагалонг** с панели **Project** в **инспектор** в **объект, чтобы тагалонг** свойство.
...
Вот наше решение, от **тагалонгактион. CS**:
