---
title: 302. HoloLens (1-го поколения) и Azure — компьютерное зрение
...
2.  теперь необходимо указать имя Project Unity. Вставка **MR_ComputerVision**. Убедитесь, что для типа проекта задано значение **3D**. Задайте для **расположения нужное расположение** (Помните, что ближе к корневым каталогам лучше). Затем нажмите кнопку **создать проект**.
...
4.  затем перейдите в **файл > сборка Параметры** а затем выберите **универсальная платформа Windows**, а затем нажмите кнопку **коммутатора платформы** , чтобы применить выбранные элементы.
    ...
    ![создание Параметры окна, переключение платформы в UWP.](images/AzureLabs-Lab2-10.png)
...
5.  несмотря на то, что в **файле > сборка Параметры** и убедитесь, что:
    ...
    6. Сохраните сцену и добавьте ее в сборку.
        ...
        3. Откройте только что созданную папку **сцены** , а затем в поле *имя файла*: введите **MR_ComputerVisionScene**, а затем нажмите кнопку **сохранить**.
            ...
            > Помните, что сцены Unity необходимо сохранять в папке *Assets* , так как они должны быть связаны с Project Unity. Создание папки «сцены» (и других аналогичных папок) — типичный способ структуризации проекта Unity.
...
3.  перетащите **LabelText** с *панели иерархия* в *папку Asset* в области *Project*. Это сделает **LabelText** prefab, чтобы его можно было создать в коде.
...
3.  Дважды щелкните новый скрипт *ресултслабел* , чтобы открыть его с помощью **Visual Studio**.
...
10. перетащите объект с именем **LabelText** из *папки assets* на *панели Project* в гнездо с именем **Label Prefab**, как показано на рисунке ниже. 
...
1.  перейдите в раздел *сборка Параметры*  -  **файл > сборка Параметры...**
