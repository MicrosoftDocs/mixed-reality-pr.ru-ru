---
title: 304. HoloLens (1-го поколения) и Azure — распознавание лиц
...
ms.locfileid: "115219565"
---
# <a name="hololens-1st-gen-and-azure-304-face-recognition"></a>HoloLens (1 gen) и Azure 304: распознавание лиц
...
1. используйте **жест касания** , чтобы начать запись изображения с помощью встроенной HoloLens камеры. 
...
1.  Откройте папку **персонмакер** и дважды щелкните *решение персонмакер* , чтобы открыть его с помощью *Visual Studio*.
...
2.  После открытия *решения персонмакер* убедитесь в том, что:
    ...
    4.  также может потребоваться *восстановление пакетов NuGet* (щелкните правой кнопкой мыши *решение* и выберите команду **восстановить NuGet пакетов**).
...
9.  нажмите кнопку **создать и откройте папку** , чтобы открыть Windows Explorer в папке, связанной с пользователем. Добавьте в папку десять изображений (10). Эти файлы должны иметь формат *JPG* или *PNG* .
...
2.  теперь необходимо указать имя Project Unity. Вставка **MR_FaceRecognition**. Убедитесь, что для типа проекта задано значение **3D**. Задайте для **расположения нужное расположение** (Помните, что ближе к корневым каталогам лучше). Затем нажмите кнопку **создать проект**.
...
4.  затем перейдите в раздел **файл > сборка Параметры** и переключите платформу на **универсальная платформа Windows**, нажав кнопку **коммутатора платформы** .

    ![создание Параметры окна, переключение платформы в UWP.](images/AzureLabs-Lab4-11.png)

5.  перейдите в раздел **файл > сборка Параметры** и убедитесь в том, что:
...
10. сохраните сцену и Project (**файл > сохранить сцену/файл > сохранить проект**).
...
Чтобы создать класс *фацеаналисис* , сделайте следующее:
 ...
 1. щелкните правой кнопкой мыши *папку assets* , расположенную на панели Project, а затем выберите **создать**  >  **папку**. Вызовите **скрипты** папки. 
...
4.  дважды щелкните новый скрипт *фацеаналисис* , чтобы открыть его с помощью Visual Studio 2017.
...
15.  в редакторе Unity перетащите сценарий фацеаналисис из папки scripts (скрипты) на Project panel в объект основной камеры на *панели иерархия*. Новый компонент скрипта будет добавлен в основную камеру. 
...
2.  дважды щелкните новый скрипт *имажекаптуре* , чтобы открыть его с помощью Visual Studio 2017.
...
5.  добавьте методы *спящего режима ()* и *Start ()* , необходимые для инициализироватьи класса, и разрешите HoloLens захватить жесты пользователя:
...
2.  последовательно выберите файл, сборка Параметры, щелкните добавить открытые сцены.
...
7.  откройте папку приложения, а затем откройте новое решение Project (как показано выше, MR_FaceRecognition. sln).
