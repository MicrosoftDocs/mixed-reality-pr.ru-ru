---
title: 308. HoloLens (1-го поколения) и Azure — уведомления на разных устройствах
description: пройдите этот курс, чтобы узнать, как реализовать центры уведомлений azure, функции azure и служба хранилища Azure и таблицы в приложении смешанной реальности.
author: drneil
ms.author: jemccull
ms.date: 07/04/2018
ms.topic: article
...
ms.locfileid: "115205855"
---
# <a name="hololens-1st-gen-and-azure-308-cross-device-notifications"></a>HoloLens (1 gen) и Azure 308: уведомления между устройствами
...
1.  Перейдите на [портал регистрации приложений](https://apps.dev.microsoft.com).
    ...
    > это **должна** быть учетная запись майкрософт, которая использовалась в предыдущей [главе](#chapter-1---create-an-application-on-the-microsoft-developer-portal)с порталом разработчика для Windows Store.
...
2.  Войдя в систему, щелкните New ( **создать** ) в левом верхнем углу, а затем найдите **Центр уведомлений** и нажмите клавишу **_Ввод_**.

    ![Поиск центра уведомлений](images/AzureLabs-Lab8-06.png)

    > [!NOTE] 
    > Слово ***New** _ может быть заменено на "_ * создать ресурс * *" в новых порталах.
...
после создания экземпляра службы концентраторов уведомлений вернитесь на портал azure, где вы создадите службу таблиц Azure, создав ресурс служба хранилища.

1.  Если вы еще не вошли в систему, войдите на [портал Azure](https://portal.azure.com).

2.  после входа в систему щелкните **создать** в левом верхнем углу и найдите **учетную запись служба хранилища** и нажмите клавишу **ввод**.

    > [!NOTE] 
    > Слово ***New** _ может быть заменено на "_ * создать ресурс * *" в новых порталах.

3.  выберите **служба хранилища учетная запись — blob-объект, файл, таблица, очередь** из списка.
...
5.  После нажатия кнопки **создать** появится панель:
    ...
    3.  для **типа учетной записи** в раскрывающемся меню выберите **служба хранилища (общего назначения** версии 1).
...
8.  Нажмите кнопку " **Переход к ресурсу** " в уведомлении, чтобы изучить новый экземпляр службы. будет выполнен переход на новую страницу обзора экземпляра службы служба хранилища.
...
1.  откройте **Блокнот** (нажмите Windows ключ и введите блокнот).

    ![открыть Блокнот](images/AzureLabs-Lab8-31.png)

2.  после открытия Блокнот вставьте приведенную ниже структуру JSON. После этого сохраните его на рабочем столе, как **project.js**. Важно правильно называть имя. Убедитесь, что у него **нет .txt** расширения файла. Этот файл определяет библиотеки, которые будет использовать функция, если вы использовали NuGet она будет выглядеть знакомо.
...
14. Откроется окно **http-размещения** , в котором необходимо настроить функцию (см. рисунок ниже для изображения).
    ...
    5.  в разделе **подключение к учетной записи служба хранилища** используйте раскрывающееся меню и выберите в нем свою учетную запись хранения. Если это не так, щелкните **новую** гиперссылку рядом с заголовком раздела, чтобы отобразить другую панель, в которой должна быть указана учетная запись хранения.
...
20. Панель будет продвигаться справа. на этой панели щелкните **Upload** и отобразится обозреватель файлов.

21. перейдите к и щелкните **project.js** файл, созданный в **Блокнот** ранее, а затем нажмите кнопку **открыть** . Этот файл определяет библиотеки, которые будет использовать функция.
...
28. вернувшись в **стандартный редактор**, в разделе **входные данные** щелкните **таблица Azure служба хранилища (таблица)**. 
...
29. Убедитесь в следующем совпадении со **своими** сведениями, так как они могут отличаться (ниже приведены действия).
    ...
    1.  **имя таблицы**: имя таблицы, созданной в служба хранилища Azure службы таблиц.

    2.  **подключение к учетной записи служба хранилища.** щелкните **создать**, которое отображается рядом с раскрывающимся меню, и справа от окна появится панель.

        ![новое хранилище](images/AzureLabs-Lab8-48.png)

        1.  выберите **учетную запись служба хранилища**, созданную ранее для размещения **приложений функции.**

        2. вы увидите, что значение подключения **учетной записи служба хранилища** создано.
...
## <a name="chapter-7---set-up-desktop-unity-project"></a>Глава 7. Настройка Project Unity для настольных систем
...
2.  необходимо указать имя Project Unity, вставить **унитидесктопнотифхуб**. Убедитесь, что для типа проекта задано значение **3D**. Задайте для **расположения нужное расположение** (Помните, что ближе к корневым каталогам лучше). Затем нажмите кнопку **создать проект**.
...
4.  затем перейдите к   >  **Параметры сборка** файла и выберите **универсальная платформа Windows**, а затем нажмите кнопку **коммутатора платформы** , чтобы применить выбранные элементы.
...
6.  в том же окне нажмите кнопку **Параметры игрока** , чтобы открыть связанную панель в пространстве, где находится **инспектор** .
...
10. сохраните сцену и Project **файл**  >  **сохранить сцену/файл**  >  **сохранить Project**.
...
вы будете использовать служба хранилища Azure для Unity (который сам использует пакет SDK для .net для Azure). дополнительные сведения см. по [ссылке служба хранилища Azure для Unity](/sandbox/gamedev/unity/azure-storage-unity).
...
Чтобы импортировать пакет SDK в собственный проект, убедитесь, что вы скачали последнюю версию [**. пакет unitypackage**](https://aka.ms/azstorage-unitysdk) из GitHub. Затем сделайте следующее.

1.  Добавьте **. пакет unitypackage** в Unity с помощью команды меню **\> \> настраиваемый пакет импорт активов** .

2.  в появившемся окне **импорт пакета Unity** можно выбрать все в разделе * *_подключаемый модуль_ \> * служба хранилища * * *.  Снимите флажок все остальное, так как он не требуется для этого курса.

    ![Импорт в пакет](images/AzureLabs-Lab8-61.png)

3.  Нажмите кнопку ***Импорт*** , чтобы добавить элементы в проект.

4.  перейдите в папку **служба хранилища** в разделе **подключаемые модули** в представлении Project и выберите *только* следующие подключаемые модули:
...
7.  установите флажок **не обрабатывать** в разделе **Параметры платформы** и нажмите кнопку **_применить_**.
...
1.  щелкните правой кнопкой мыши папку **Asset** , расположенную на панели Project, **создайте**  >  **папку**. Назовите папку **Scripts**.
...
4.  дважды щелкните сценарий, чтобы открыть его в Visual Studio 2017.
...
6.  В классе вставьте следующие переменные:
    ...
    > [!NOTE] 
    > замените значение **accountName** на имя службы служба хранилища Azure и значение **accountKey** значением ключа, которое находится в службе служба хранилища Azure, на портале Azure (см. изображение ниже). 
    >
...
## <a name="chapter-10---create-the-cloudscene-class-in-the-desktop-unity-project"></a>Глава 10. Создание класса Клаудсцене в классе Unity для настольных систем Project
...
## <a name="chapter-11---build-the-desktop-project-to-uwp"></a>глава 11. создание рабочего стола Project в UWP
...
## <a name="chapter-12---set-up-mixed-reality-unity-project"></a>Глава 12. Настройка Unity в смешанной реальности Project
...
2.  теперь необходимо указать имя Project Unity, вставить **унитимрнотифхуб**. Убедитесь, что для типа проекта задано значение **3D**. Задайте для **расположения нужное расположение** (Помните, что ближе к корневым каталогам лучше). Затем нажмите кнопку **создать проект**.
...
7.  На этой панели необходимо проверить несколько параметров:
    ...
    2.  далее на панели в **XR Параметры** (находится под **Параметры публикации**), **поддерживаемая виртуальная реальность** tick, убедитесь, что добавлен **пакет SDK Windows Mixed Reality**

        ![обновить параметры XR](images/AzureLabs-Lab8-88.png)        

    3.  на вкладке **публикация Параметры** в разделе **возможности**:

        - **InternetClient**;           

            ![Клиент Tick Internet](images/AzureLabs-Lab8-89.png)

8.  вернувшись в **Параметры сборки**. **проекты C# для Unity** больше не заменяются: установите флажок рядом с этим.

9.  после внесения этих изменений закройте окно сборка Параметры.
...
### <a name="chapter-13---importing-the-dlls-in-the-mixed-reality-unity-project"></a>Глава 13. импорт библиотек DLL в Project Unity в смешанной реальности

вы будете использовать служба хранилища Azure для библиотеки Unity (которая использует пакет SDK для .net для Azure). перейдите по этой [ссылке, чтобы использовать служба хранилища Azure с Unity](/sandbox/gamedev/unity/azure-storage-unity).
В Unity существует известная ошибка, которая требует перенастройки подключаемых модулей после импорта. Эти действия (4-7 в этом разделе) больше не понадобятся после устранения ошибки.
...
2.  в появившемся окне **импорт пакета Unity** можно выбрать все в разделе **подключаемый модуль**  >  **служба хранилища**.
...
Класс **таблетосцене** идентичен описанному в [главе 9](#chapter-9---create-the-tabletoscene-class-in-the-desktop-unity-project). создайте тот же класс в Project Unity в смешанной реальности, как описано в [главе 9](#chapter-9---create-the-tabletoscene-class-in-the-desktop-unity-project).

После завершения этой главы в обоих **проектах Unity** этот класс будет настроен на основной камере.

## <a name="chapter-15---creating-the-notificationreceiver-class-in-the-mixed-reality-unity-project"></a>Глава 15. Создание класса Нотификатионрецеивер в Unity в среде Mixed Reality Project
...
## <a name="chapter-16---build-the-mixed-reality-project-to-uwp"></a>глава 16. создание Project смешанной реальности в UWP
...
2.  в меню **Параметры сборки** убедитесь, что **проекты C# для Unity*** являются тактовыми (что позволит изменять скрипты в этом проекте после сборки).
...
> [!WARNING] 
> помните, что после добавления следующих пакетов NuGet (и раскомментируйте код в следующей [главе](#chapter-18---edit-unitymrnotifhub-application-notificationreceiver-class)) код, при повторном открытии в Project Unity, будет представлять ошибки. Если вы хотите вернуться и продолжить редактирование в редакторе Unity, вам потребуется комментарий, ерросоме код, а затем раскомментировать его позже, как только вы вернетесь в Visual Studio. 

после завершения сборки mixed reality перейдите к проекту mixed reality, который вы создали, и дважды щелкните файл решения (sln) в этой папке, чтобы открыть решение с Visual Studio 2017.
теперь необходимо добавить пакет **WindowsAzure. Messaging. managed** NuGet; это библиотека, которая используется для получения уведомлений из центра уведомлений.
...
3.  Выберите вкладку ***Обзор** _ и выполните поиск по слову _ * WindowsAzure. Messaging. Managed * *.

    ![Поиск пакета Windows Azure Messaging](images/AzureLabs-Lab8-103.png)

4.  Выберите результат (как показано ниже) и в окне справа установите флажок рядом с **Project**. он поместит такт в флажок рядом с **Project**, а также флажок рядом с проектом **сборка — CSharp** и **унитимрнотифхуб** .
...
2.  щелкните правой кнопкой мыши приложение UWP Project на панели обозреватель решений, перейдите к **магазину** и **свяжите приложение с магазином...**

    ![открыть ассоциацию магазина](images/AzureLabs-Lab8-105.png)

3.  появится новое окно **с именем связать приложение с хранилищем Windows**. Щелкните **Далее**.
