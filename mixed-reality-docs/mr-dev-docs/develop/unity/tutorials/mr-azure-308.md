...
---
# <a name="hololens-1st-gen-and-azure-308-cross-device-notifications"></a>HoloLens (1-й общий) и Azure 308: уведомления между устройствами
...
32. На этом этапе необходимо протестировать функцию, чтобы убедиться, что она работает. Для этого выполните следующие действия. 
...
Вы будете использовать службу хранилища Azure для Unity (которая самостоятельно использует пакет SDK для .NET для Azure). Дополнительные сведения см. по этой [ссылке о службе хранилища Azure для Unity](/sandbox/gamedev/unity/azure-storage-unity).
...
Вы будете использовать службу хранилища Azure для библиотеки Unity (которая использует пакет SDK для .NET для Azure). Перейдите по этой [ссылке, чтобы использовать службу хранилища Azure с Unity](/sandbox/gamedev/unity/azure-storage-unity).
В Unity существует известная ошибка, которая требует перенастройки подключаемых модулей после импорта. Эти действия (4-7 в этом разделе) больше не понадобятся после устранения ошибки.
...
3.  Появится новое окно **с именем связать приложение с магазином Windows**. Щелкните **Далее**.
...
5.  Найдите **имя приложения Магазина** , созданное в начале этого руководства, и выберите его. Нажмите кнопку **Далее**.
