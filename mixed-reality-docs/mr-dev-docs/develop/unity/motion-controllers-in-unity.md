...
ms.date: 12/1/2020
ms.topic: article
keywords: контроллеры движения, unity, ввод, гарнитура смешанной реальности, гарнитура windows mixed reality, гарнитура виртуальной реальности, мртк, смешанная реальность набор средств
ms.openlocfilehash: ccda5b11190e829ccc655989a6f679ef6ef647a920c01a3182548b23a3d85084
ms.sourcegitcommit: a1c086aa83d381129e62f9d8942f0fc889ffcab0
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 08/05/2021
ms.locfileid: "115216248"
---
# <a name="motion-controllers-in-unity"></a>Контроллеры движения в Unity

существует два основных способа принять меры в отношении вашего [взгляда в Unity](gaze-in-unity.md), а [жесты](../../design/gaze-and-commit.md#composite-gestures) и [контроллеры движения](../../design/motion-controllers.md) в HoloLens и иммерсивное хмд. Доступ к данным для обоих источников пространственных данных осуществляется через одни и те же API в Unity.

Unity предоставляет два основных способа доступа к данным пространственного ввода для Windows Mixed Reality. общие *входные api-интерфейсы input. "input. axis"* работают в нескольких пакетах sdk для Unity XR, тогда как API *интерактионманажер/GestureRecognizer* , относящийся к Windows Mixed Reality предоставляет полный набор пространственных входных данных.
...
диспетчер ввода Unity для контроллеров движения Windows Mixed Reality поддерживает указанные ниже идентификаторы кнопок и осей с помощью api-интерфейсов *Input. onbutton и axis* . столбец "Windows MR" относится к свойствам, доступным для типа *интерактионсаурцестате* . Каждый из этих API подробно описывается в следующих разделах.

сопоставления кнопок или идентификаторов осей для Windows Mixed Reality, как правило, соответствуют идентификаторам кнопок и осей окулус.

сопоставления идентификаторов кнопок и осей для Windows Mixed Reality отличаются от сопоставлений опенвр двумя способами:
...
<table>
<tr>
<th rowspan="2">Входные данные </th><th colspan="2"><a href="motion-controllers-in-unity.md#common-unity-apis-inputgetbuttongetaxis">Распространенные API Unity</a><br />(Входные данные. Кнопка/ось) </th><th rowspan="2"><a href="motion-controllers-in-unity.md#windows-specific-apis-xrwsainput">Windows API ввода, характерный для MR</a><br />XR. Головк. Входной</th>
</tr><tr>
...
| инпутфеатуреусаже | HP reverbы G2 Controller (Опенкср) | HoloLens Рука (Опенкср) |
| ---- | ---- | ---- |
...
Windows Mixed Reality поддерживает контроллеры движения в различных форм-факторах. Структура каждого контроллера различает свое отношение между положением пользователя и естественным направлением "вперед", которое приложения должны использовать для указания при подготовке к просмотру контроллера.
...
подсистема **захвата** представляет расположение карманных пользователей, обнаруженных HoloLens или удержанием контроллера движения.
...
* **Позиция захвата**: Карманный центроид при удержании контроллера естественным образом настраивается влево или вправо для центрирования места внутри захвата. в Windows Mixed Reality контроллере движения это расположение обычно соответствует кнопке курса.
...
в настоящее время указатель a доступен в Unity только через Windowsный API-интерфейс MR, *саурцестате. саурцепосе. трижетпоситион/вращение*, передавая *интерактионсаурценоде. pointer* в качестве аргумента.
...
если пользователь перемещает контроллеры за пределы поля гарнитуры, Windows в большинстве случаев будет продолжать вычислять положение контроллеров. Когда контроллер потеряет Визуальное отслеживание на достаточно длинном уровне, позиции контроллера будут отбрасываться на позиции приблизительной точности.
...
например, чтобы связать кнопку триггера левого контроллера движения с действием отправки, перейдите в меню **правка > Project Параметры > вход** в Unity и разверните свойства раздела отправить в разделе оси. Измените **положительную кнопку** или свойство **положительной кнопки Alt** , чтобы прочесть **игровую кнопку 14**, например:
...
## <a name="windows-specific-apis-xrwsainput"></a>Windows интерфейсы api (XR. Головк. Входной
...
* когда пользователь нажимает кнопку на контроллере, может быть около 20 мс беспроводной задержки по сравнению с Bluetooth до того, как система получит нажатие.
