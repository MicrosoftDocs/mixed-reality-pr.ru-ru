---
title: 250 MR-HoloLens и впечатляющие головные телефоны
description: выполните приведенное ниже пошаговое руководство по программированию с помощью Unity, Visual Studio, HoloLens и Windows Mixed Reality гарнитуры, чтобы получить сведения об общих голограммах между устройствами смешанной реальности.
author: keveleigh
ms.author: kurtie
ms.date: 10/22/2019
ms.topic: article
...
* создайте общий опыт, в котором HoloLens пользователи поделили впечатляющие гарнитуры с помощью простых головоломок.
...
* Windows 10 пк с [необходимыми средствами разработки](../develop/install-the-tools.md) и [настроенными для поддержки Windows Mixed Reality иммерсивное гарнитура](/windows/mixed-reality/enthusiast-guide/windows-mixed-reality-minimum-pc-hardware-compatibility-guidelines).
...
>[!NOTE]
>Если вы хотите просмотреть исходный код перед загрузкой, он будет [доступен на GitHub](https://github.com/Microsoft/MixedReality250).
...
приложение, которое показывает голограмму на HoloLens или Windows Mixed Reality иммерсивное гарнитура.
...
* Убедитесь, что Смешанная реальность включена в Unity.
    * откройте диалоговое окно "параметры сборки" (**Control + Shift + B** или **файл > сборка Параметры...**).
    * выберите **универсальная платформа Windows** а затем выберите **параметр платформа**.
    * выберите **изменить>проигрыватель Параметры**.
    * на панели **инспектора** с правой стороны разверните узел **XR Параметры**.
...
* Экспорт в Visual Studio
    * откройте меню сборка (**Control + Shift + B** или **файл > сборка Параметры**).
...
* Сборка из Visual Studio
    ...
    * Щелкните стрелку рядом с пунктом **локальный компьютер** и выберите **устройство** для развертывания HoloLens
...
На панели проект перейдите к **ассетс\холотулкит\инпут\скриптс\утилитиес** и дважды щелкните **микседреалитикамераманажер. CS** , чтобы открыть его.

**Обзор:** Микседреалитикамераманажер. CS — это простой сценарий, который корректирует уровень качества и параметры фона на основе устройства. ключевым здесь является холографиксеттингс. исдисплайопакуе, который позволяет сценарию определить, является ли устройство HoloLensом (исдисплайопакуе возвращает значение false) или иммерсивное гарнитура (исдисплайопакуе возвращает true).
...
показывает, как обрабатывает входные данные для Windows Mixed Reality приложения.

### <a name="what-we-will-build"></a>Что будет построено

основываясь на приложении, описанном в разделе 1, мы добавим функции, позволяющие пользователю выбрать голограмму и поместить ее на реальной поверхности HoloLens или в виртуальной таблице на современном гарнитуре.

**Обновитель ввода:** в HoloLens жестом выбора является **воздушный нажим**. На впечатляющих гарнитурах мы будем **использовать кнопку на** контроллере Xbox. Дополнительные сведения см. в [обзоре модели взаимодействия](../design/interaction-fundamentals.md).
...
**Сценарий 1 — Гамепадинпут. CS**

На панели проект перейдите к **ассетс\холотулкит\инпут\скриптс\инпутсаурцес** и дважды щелкните **гамепадинпут. CS** , чтобы открыть его. В том же пути на панели проекта также дважды щелкните **интерактионсаурцеинпутсаурце. CS**.
...
**Сценарий 2 — Таптоплаце. CS**

На панели проект перейдите к **ассетс\холотулкит\спатиалмаппинг\скриптс** и дважды щелкните **таптоплаце. CS** , чтобы открыть его.

первое, что многие разработчики хотят реализовать при создании приложения holographic, — это перемещение Голограммы с входом жеста. Таким образом, мы предоставили тщательное комментирование этого сценария. Для этого руководства стоит выделить несколько моментов.

Во первых, обратите внимание, что Таптоплаце реализует Иинпуткликкхандлер. Иинпуткликкхандлер предоставляет функции, обрабатывающие событие Инпуткликкед, вызванное Гамепадинпут. cs или Интерактионсаурцеинпутсаурце. cs. Онинпуткликкед вызывается, когда Басеинпутсаурце обнаруживает щелчок, пока объект с Таптоплаце находится в фокусе. либо аиртаппинг на HoloLens, либо нажав кнопку на контроллере Xbox, вы запустите событие.
...
На этот раз можно выбрать остров, чтобы переместить его. на HoloLens можно переместить остров в реальную поверхность. В поувлекательной гарнитуре можно переместить остров в виртуальную таблицу, которую мы добавили.
...
Наш проект будет преобразован в проект с несколькими проектами. Мы добавим пользовательский интерфейс и логику для размещения сеансов или подключения к ним. HoloLens пользователи будут видеть друг друга в сеансе с облаками в своих головах, а у впечатляющих головных пользователей есть облака, расположенные рядом с точкой привязки. пользователи в захватывающих гарнитурах увидят HoloLens пользователей относительно источника сцены. HoloLens пользователи будут видеть голограмму острова в том же месте. ключевое замечание заключается в том, что пользователи в впечатляющих головных гарнитурах не будут включены в эту главу, но будут вести себя так же, как HoloLens, с птицм глазным представлением острова.
...
На панели проект перейдите к **ассетс\апппрефабс\суппорт\шарингвисунет\скриптс** и дважды щелкните **унетанчорманажер. CS**. возможность одного HoloLens обмениваться данными отслеживания с другими HoloLens например, чтобы оба устройства могли совместно использовать одно и то же пространство, приближается к magical. Мощь смешанной реальности вступает в активность, когда два или более пользователей могут совместно работать с одними и теми же цифровыми данными.
...
на этот раз пользователь с HoloLens будет размещать сеанс с помощью кнопки **запустить сеанс** в пользовательском интерфейсе. другие пользователи, как на HoloLens, так и в иммерсивного гарнитуре, будут выбирать сеанс, а затем кнопку **присоединить сеанс** в пользовательском интерфейсе. если у вас есть несколько человек с HoloLens устройствами, они будут иметь красные облака в своих головах. также будет отображаться синий облако для каждой иммерсивного гарнитуры, но синие облака не будут выставляться над гарнитурами, так как гарнитуры не пытаются найти то же пространство координат мира, что и HoloLensные устройства.
...
Мы будем обновлять приложение, чтобы разместить впечатляющие пользователи гарнитуры по острову с иммерсивное представлением. HoloLens пользователи по-прежнему будут иметь представление о острове с высоты птичьего полета. Пользователи каждого типа устройств могут видеть других пользователей по мере их появления в мире. например, увлекательные пользователи с наушниками могут видеть другие аватары на других путях, и они видят HoloLens пользователей как гигантские облака выше острова. для пользователей с головной гарнитурой также будет отображаться курсор HoloLensго пользователя, если пользователь HoloLens просматривает остров. HoloLens пользователи увидят аватар по острову, который будет представлять каждого иммерсивного пользователя гарнитуры.
...
На панели проект перейдите к **ассетс\апппрефабс\суппорт\скриптс\гамелогик** и дважды щелкните **микседреалитителепорт. CS**.
...
Второй скрипт вызывает Сетворлдпоситион. Давайте взглянем на этот сценарий. На панели проект перейдите к **ассетс\апппрефабс\суппорт\скриптс\гамелогик** и дважды щелкните **телепортскрипт. CS**.
...
опять же, пользователь с HoloLens будет размещать сеанс. Другие пользователи будут присоединяться к сеансу. Приложение поместит первых трех пользователей, чтобы присоединиться к увлекательной гарнитуре по одному из трех путей на самом острове. В этом разделе вы можете изучить остров.

Сведения для уведомления:

1. вы видите лица в облаках, которые помогают высмотреть, какое направление пользователь HoloLens.
...
3. если HoloLens пользователь просматривает остров, то иммерсивное пользователи могут видеть свой курсор.
4. облака, представляющие HoloLensные пользователем тени.
...
основываясь на главе 4, когда пользователь с впечатляющим головным телефоном приближается к головоломке по острову, HoloLens пользователи получат подсказку с советом о головоломке. После того, как все пользователи с головным телефоном получит свои головоломки и появятся на "готовой панели" в Rocket комнате, Rocket запустится.
...
Теперь давайте взглянем на Левелконтрол. cs. Этот сценарий является ядром логики игры и поддерживает состояние игры. Поскольку это многопользовательская игра, использующая УНЕТ, нам нужно понять, как потоки данных, как минимум, достаточно для изменения этого руководства. Более полный обзор УНЕТ см. в документации Unity.

На панели проект перейдите к **ассетс\апппрефабс\суппорт\скриптс\гамелогик** и дважды щелкните **левелконтрол. CS**.
...
Вернувшись в Левелконтрол. cs, Взгляните на Сетгоалиндекс. Здесь мы назначим значение значения в синклист (Атгоал). Помните, что в контексте узла мы делаем это. Как и команда, RPC — это то, что может выдать узел, что приведет к тому, что все клиенты будут запускать некоторый код. Здесь мы вызываем "Рпкчеккаллгоалс". Каждый клиент будет отдельно проверять, установлены ли все три Атгоалс, и если да, запустите Rocket.

### <a name="enjoy-your-progress"></a>Наслаждайтесь ходом выполнения

Основываясь на предыдущей главе, мы будем запускать сеанс, как и раньше. на этот раз, когда пользователи в самой увлекательной гарнитуре появятся в "дверце" на пути, появится всплывающая подсказка, которая может видеть только HoloLens пользователей. HoloLens пользователи несут ответственность за передачу этой подсказки пользователям в иммерсивное гарнитуру. Rocket запустится, когда каждый Аватар пойдет на соответствующую коричневую панель в о вулканах. Сцена будет сброшена через 60 секунд, что позволит выполнить ее снова.
