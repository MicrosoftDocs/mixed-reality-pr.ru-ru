...
Таким образом, разработчикам следует убедиться, что для этих пользователей предусмотрена соответствующая резервная поддержка. На странице [отслеживания взгляда на DirectX](../develop/native/gaze-in-directx.md#fallback-when-eye-tracking-is-not-available) мы объясним API-интерфейсы, необходимые для определения того, доступны ли данные отслеживания взгляда. 
