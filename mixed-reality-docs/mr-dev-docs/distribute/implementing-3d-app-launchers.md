...
description: узнайте, как создавать программы запуска и логотипы трехмерных приложений для Windows Mixed Reality приложений UWP и игр на HoloLens и на наушниках VR.
author: thmignon
ms.author: thmignon
ms.date: 07/12/2018
ms.topic: article
...
ms.locfileid: "115196428"
---
# <a name="implement-3d-app-launchers-uwp-apps"></a>Реализация средств запуска трехмерных приложений (приложения UWP)

> [!NOTE]
> эта функция была добавлена как часть обновления 2017 в RS3 (creator) для впечатляющих головных телефонов и поддерживается HoloLens с обновлением Windows 10 апреля 2018. убедитесь, что приложение предназначено для версии Windows SDK больше или равно 10.0.16299 на впечатляющих гарнитурах и 10.0.17125 на HoloLens. последнюю Windows SDK можно найти [здесь](https://developer.microsoft.com/windows/downloads/windows-10-sdk).

[Windows Mixed Reality home](../discover/navigating-the-windows-mixed-reality-home.md) является отправной точкой, где пользователи начинают работу перед запуском приложений. при создании приложения UWP для Windows Mixed Reality по умолчанию приложения запускаются в виде двумерных планшетов с логотипом своего приложения. при разработке возможностей для Windows Mixed Reality трехмерное средство запуска может быть определено для переопределения 2d-запуска по умолчанию для приложения. в общем случае трехмерные запуски рекомендуются для запуска впечатляющих приложений, которые выводят пользователей из Windows Mixed Reality дома. Если приложение активировано на месте, предпочтительнее использовать по умолчанию 2D Launcher. Можно также создать [трехмерную глубину ссылки (секондаритиле)](#3d-deep-links-secondarytiles) в виде трехмерного модуля запуска для содержимого в ДВУХМЕРНОМ приложении UWP.
...
Элемент Микседреалитимодел принимает путь к файлу, указывающий на трехмерный ресурс, хранящийся в пакете приложения. сейчас поддерживаются только объемные модели, которые доставляются с помощью формата файла glba и созданы в соответствии с [инструкциями по созданию Windows Mixed Reality трехмерных ресурсов](creating-3d-models-for-use-in-the-windows-mixed-reality-home.md) . Ресурсы должны храниться в пакете приложения, и анимация в настоящее время не поддерживается. если параметр "Path" пуст Windows вместо 3d-запуска будет отображаться 2d. **Примечание** . ресурс. GLBA должен быть помечен как "Content" в параметрах сборки перед сборкой и запуском приложения.
...
поддержка атрибута ограничивающего прямоугольника поступает вместе с Windows RS4 update в качестве свойства элемента микседреалитимодел. Чтобы сначала определить ограничивающий прямоугольник в верхней части манифеста приложения, добавьте схему uap6 и включите ее как игнорируемые пространства имен:
...
> [!NOTE]
> Эта функция была добавлена в составе обновления 2017 (RS3) для "иммерсивное" (VR), а также в рамках обновления за Апрель 2018 (RS4) для HoloLens. убедитесь, что приложение предназначено для версии Windows SDK, которое больше или равно 10.0.16299 на наушниках (VR) и 10.0.17125 на HoloLens. последнюю Windows SDK можно найти [здесь](https://developer.microsoft.com/windows/downloads/windows-10-sdk).

>[!IMPORTANT]
>Трехмерные глубокие ссылки (secondaryTiles) работают только с 2D-приложениями UWP. однако можно создать [средство запуска для 3d-приложения](implementing-3d-app-launchers.md) , чтобы запустить эксклюзивное приложение из Windows Mixed Reality home.

2d-приложения можно улучшить для Windows Mixed Reality, добавив возможность размещения трехмерных моделей из приложения в [Windows Mixed Reality home](../discover/navigating-the-windows-mixed-reality-home.md) в качестве глубоких ссылок на содержимое в двухмерном приложении, подобно [2d-дополнительным плиткам](/windows/uwp/controls-and-patterns/tiles-and-notifications-secondary-tiles) на Windows меню. Например, можно создать 360 °, которые непосредственно связываются с приложением 360 ° Photo Viewer, или позволить пользователям размещать трехмерное содержимое из коллекции ресурсов, открывающей страницу сведений об авторе. Это всего лишь два способа расширить функциональные возможности 2D-приложения с помощью трехмерного содержимого.
...
> [!NOTE]
> эта функция будет поддерживаться Windows обновления RS4. убедитесь, что приложение предназначено для версии Windows SDK больше или равно 10.0.17125, если вы планируете использовать эту функцию
...
2. Нет: при выборе пользователем 3D Секондаритиле ничего не происходит, и приложение не активируется.
...
трехмерные ссылки (secondaryTiles) можно создавать только во время отображения представления на гарнитуре Windows Mixed Reality. если представление не представлено на гарнитуре Windows Mixed Reality, рекомендуется корректно обработать его, либо скрыть точку входа, либо показать сообщение об ошибке. Это можно проверить, выполнив запрос к [искуррентвиевпресентедонхолографик ()](/uwp/api/windows.applicationmodel.preview.holographic.holographicapplicationpreview#Windows_ApplicationModel_Preview_Holographic_HolographicApplicationPreview_IsCurrentViewPresentedOnHolographicDisplay_).

## <a name="tile-notifications"></a>Уведомления на плитках

Уведомления плитки сейчас не поддерживают отправку обновлений в 3D-ресурс. Это означает, что разработчики не могут выполнять следующие действия:
